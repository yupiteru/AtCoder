...
namespace Program
{
    public static class DPD
    {
        static public void Solve()
        {
            var N = NextInt;
            var W = NextInt;
            var wvList = Repeat(0, N).Select(_ => new { w = NextInt, v = NextLong }).ToList();

            var dp = new long[N, W + 1];
            dp[0, wvList[0].w] = wvList[0].v;
            for (int i = 1; i < N; i++)
            {
                for (int j = 0; j <= W; j++)
                {
                    ...
                    if (j >= wvList[i].w)
                    {
                        dp[i, j] = Max(dp[i, j], dp[i - 1, j - wvList[i].w] + wvList[i].v);
                    }
                }
            }
            Console.WriteLine(Enumerable.Range(0, W + 1).Select(e => dp[N - 1, e]).Max());
