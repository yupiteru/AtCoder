...
namespace Program
{
    public static class DPC
    {
        ...
        {
            var N = NextInt;
            var abcList = Repeat(0, N).Select(_ => new { a = NextLong, b = NextLong, c = NextLong }).ToList();
            ...
            for (int i = 1; i < N; i++)
            {
                dp[i, A] = Max(dp[i - 1, B], dp[i - 1, C]) + abcList[i].a;
                dp[i, B] = Max(dp[i - 1, A], dp[i - 1, C]) + abcList[i].b;
                dp[i, C] = Max(dp[i - 1, A], dp[i - 1, B]) + abcList[i].c;
            }
            Console.WriteLine(Max(Max(dp[N - 1, A], dp[N - 1, B]), dp[N - 1, C]));
