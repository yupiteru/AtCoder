...
namespace Program
{
    public static class DPC
    {
        ...
        {
            var N = NN;
            var abc = Repeat(0, N).Select(_ => new { a = NN, b = NN, c = NN }).ToArray();
            var dp = new long[N + 1, 3];
            for (var i = 1; i <= N; i++)
            {
                dp[i, 0] = Max(dp[i - 1, 1], dp[i - 1, 2]) + abc[i - 1].a;
                dp[i, 1] = Max(dp[i - 1, 0], dp[i - 1, 2]) + abc[i - 1].b;
                dp[i, 2] = Max(dp[i - 1, 0], dp[i - 1, 1]) + abc[i - 1].c;
            }
            Console.WriteLine(Max(Max(dp[N, 0], dp[N, 1]), dp[N, 2]));
        }
