...
namespace Program
{
    public static class DPE
    {
        static public void Solve()
        {
            var N = NextLong;
            var W = NextLong;
            var wv = Repeat(0, N).Select(_ => new { w = NextLong, v = NextLong }).ToList();

            var dp = new long?[wv.Sum(e => e.v) + 1];
            dp[0] = 0;
            foreach (var item in wv)
            {
                for (var i = dp.Length - 1; i >= item.v; i--)
                {
                    if (dp[i - item.v].HasValue)
                    {
                        if (dp[i].HasValue) dp[i] = Min(dp[i].Value, dp[i - item.v].Value + item.w);
                        else dp[i] = dp[i - item.v].Value + item.w;
                    }
                }
            }
            Console.WriteLine(dp.Select((x, y) => new { x, y }).Where(e => e.x <= W).Last().y);
