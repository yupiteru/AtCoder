...
namespace Program
{
    public static class AISING2019D
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var Q = NN;
            var AList = NNList(N).ToList();
            var XList = NNList(Q);
            var takahasiList = new List<long>();
            for (var i = 0; i < N; i++)
            {
                if (i % 2 == 0) takahasiList.Add(AList[(int)N - i - 1]);
            }
            takahasiList.Reverse();
            var ruisekiTakahasiList = new long[takahasiList.Count + 1];
            for (var i = 1; i <= takahasiList.Count; i++)
            {
                ruisekiTakahasiList[i] = ruisekiTakahasiList[i - 1] + takahasiList[i - 1];
            }
            var AListRevRuiseki = new long[N + 1];
            var AListRev = AList.ToArray().Reverse().ToArray();
            for (var i = 1; i <= N; i++)
            {
                AListRevRuiseki[i] = AListRevRuiseki[i - 1] + AListRev[i - 1];
            }
            foreach (var item in XList)
            {
                var idx = AList.BinarySearch(item);
                if (idx < 0)
                {
                    var tmp = ~idx;
                    if (tmp == N) idx = (int)N - 1;
                    else if (tmp == 0) idx = 0;
                    else
                    {
                        var a = AList[tmp];
                        var b = AList[tmp - 1];
                        if (Abs(b - item) <= Abs(a - item)) idx = tmp - 1;
                        else idx = tmp;
                    }
                }
                var left = idx - 1;
                ...
                while (right - left > 1)
                {
                    ...
                    {
                        var mid2 = (right2 + left2) / 2;
                        if (Abs(AList[idx - mid2] - item) <= Abs(item - AList[(int)mid])) left2 = mid2;
                        else right2 = mid2;
                    }
                    if (mid - idx + left2 + 1 >= N - mid) right = mid;
                    else left = (int)mid;
                }
                var firstCnt = N - right;
                Console.WriteLine(AListRevRuiseki[firstCnt] + ruisekiTakahasiList[takahasiList.Count - firstCnt]);
            }
        }
