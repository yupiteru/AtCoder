...
namespace Program
{
    public static class SUMITRUST2019D
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var S = NS;
            var pool1 = new HashSet<long>();
            var pool2 = new HashSet<long>();
            var ans = new HashSet<long>();
            foreach (var item in S)
            {
                foreach (var item2 in pool2)
                {
                    ans.Add(item2 * 10000 + item - '0');
                }
                foreach (var item2 in pool1)
                {
                    pool2.Add(item2 * 100 + item - '0');
                }
                pool1.Add(item - '0');
            }
