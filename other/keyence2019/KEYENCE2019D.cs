...
namespace Program
{
    public static class KEYENCE2019D
    {
        ...
        static public void Solve()
        {
            ...
            var AList = NNList(N);
            var BList = NNList(M);
            if (AList.Distinct().Count() != AList.Count() || BList.Distinct().Count() != BList.Count())
            {
                Console.WriteLine(0);
                return;
            }
            var sortedA = AList.OrderByDescending(e => e).ToList();
            var sortedB = BList.OrderByDescending(e => e).ToList();
            sortedA.Add(0);
            sortedB.Add(0);
            LIB_Mod ans = 1;
            var aIdx = 0;
            ...
            for (var i = N * M; i > 0; i--)
            {
                while (sortedA[aIdx] > i) ++aIdx;
                while (sortedB[bIdx] > i) ++bIdx;
                if (sortedA[aIdx] == i && sortedB[bIdx] == i) ans *= 1;
                else if (sortedA[aIdx] != i && sortedB[bIdx] == i) ans *= aIdx;
                else if (sortedA[aIdx] == i && sortedB[bIdx] != i) ans *= bIdx;
                else
                {
                    LIB_Mod tmp = aIdx;
                    tmp *= bIdx;
