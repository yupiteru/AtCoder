...
namespace Program
{
    public static class DIVERTA2019_2C
    {
        static public void Solve()
        {
            var N = NextInt;
            var Alist = Sort(NextIntList(N));

            var fidx = 0;
            var lidx = N - 1;

            var opeList = new List<Tuple<int, int>>();
            while (true)
            {
                var f = Alist[fidx];
                var l = Alist[lidx];
                ...
                ++fidx;
                --lidx;
                if (t - Alist[fidx] > Alist[lidx] + t)
                {
                    opeList.Add(Tuple.Create(l, f));
                    ++lidx;
                    Alist[lidx] = l - f;
                }
                else
                {
                    opeList.Add(Tuple.Create(f, l));
                    --fidx;
                    Alist[fidx] = f - l;
                }
            }
