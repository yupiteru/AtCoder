...
namespace Program
{
    public static class DIVERTA2019_2D
    {
        struct I
        {
            public long g;
        ...
        {
            var N = NextLong;
            var X = Repeat(0, 2).Select(_ => NextLongList(3)).Select(e => new I { g = e[0], s = e[1], b = e[2] }).ToList();
            var A = X[0];
            var B = X[1];
            if (A.g > B.g && A.s > B.s)
            {
            ...
            var maxDonguriAtB = 0L;
            for (long i = 0; i * A.g <= N; ++i)
            {
                for (long j = 0; j * A.s <= N - i * A.g; ++j)
                {
                    var tmp = i * A.g + j * A.s;
                    if (tmp <= N)
                    {
                        ...
                        if (A.b > B.b)
                        {
                            maxDonguriAtB = Max(maxDonguriAtB, i * B.g + j * B.s + tmp2);
                        }
                        else
                        {
                            maxDonguriAtB = Max(maxDonguriAtB, i * B.g + j * B.s + (tmp2 / A.b) * B.b + tmp2 % A.b);
                        }
                    }
                    if (A.s > B.s) break;
                }
            ...
            var maxDonguri = 0L;
            for (long i = 0; i * B.g <= maxDonguriAtB; ++i)
            {
                for (long j = 0; j * B.s <= maxDonguriAtB - i * B.g; ++j)
                {
                    var tmp = i * B.g + j * B.s;
                    if (tmp <= maxDonguriAtB)
                    {
                        var tmp2 = (maxDonguriAtB - tmp);
                        if (B.b >= A.b)
                        {
                            maxDonguri = Max(maxDonguri, i * A.g + j * A.s + tmp2);
                        }
                        else
                        {
                            maxDonguri = Max(maxDonguri, i * A.g + j * A.s + (tmp2 / B.b) * A.b + tmp2 % B.b);
                        }
                    }
                    if (B.s >= A.s) break;
                }
