...
namespace Program
{
    public static class TDPCC
    {
        ...
        {
            var K = NN;
            var numOfPlayer = Pow((long)2, K);
            var RList = NNList(numOfPlayer);
            var aite = new List<long>[numOfPlayer, K + 1];
            for (var i = 1; i <= K; i++)
            {
                var cnt = Pow((long)2, i - 1);
                var list = new List<List<long>>();
                var tmpList = new List<long>();
                for (var j = 0; j < numOfPlayer; j++)
                {
                    tmpList.Add(j);
                ...
                {
                    foreach (var item in list[j])
                    {
                        aite[item, i] = new List<long>(list[j + 1]);
                    }
                    foreach (var item in list[j + 1])
                    {
                        aite[item, i] = new List<long>(list[j]);
                    }
                }
            }
            var dp = new double?[numOfPlayer, K + 1];
            for (var i = 0; i < numOfPlayer; i++)
            {
                dp[i, 0] = 1;
            }
            var func = Lambda<long, long, double>((player, k, fun) =>
            {
                if (!dp[player, k].HasValue)
                {
                    var ansSum = 0.0;
                    var allSum = 0.0;
                    foreach (var item in aite[player, k])
                    {
                        var battle = fun(item, k - 1);
                        allSum += battle;
                        ansSum += battle / (1.0 + Math.Pow(10.0, (RList[item] - RList[player]) / 400.0));
                    }
                    dp[player, k] = fun(player, k - 1) * ansSum / allSum;
                }
                return dp[player, k].Value;
            });
            for (var i = 0; i < numOfPlayer; i++)
            {
                Console.WriteLine(func(i, K));
            }
        }
