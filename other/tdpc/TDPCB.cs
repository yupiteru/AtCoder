        ...
        static public void Solve()
        {
            ...
            for (var i = A; i >= 0; i--)
            {
                for (var j = B; j >= 0; j--)
                {
                    if (i < A && j < B)
                    {
                        dp[i, j, 0] = Max(dp[i + 1, j, 1] + aList[i], dp[i, j + 1, 1] + bList[j]);
                        dp[i, j, 1] = Min(dp[i + 1, j, 0], dp[i, j + 1, 0]);
                    }
                    else if (i < A)
                    {
                        dp[i, j, 0] = dp[i + 1, j, 1] + aList[i];
                        dp[i, j, 1] = dp[i + 1, j, 0];
                    }
                    else if (j < B)
                    {
                        dp[i, j, 0] = dp[i, j + 1, 1] + bList[j];
                        dp[i, j, 1] = dp[i, j + 1, 0];
                    }
                }
            }
