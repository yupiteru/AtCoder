...
namespace Program
{
    public class M_SOLUTIONSE
    {
        static public void Solve()
        {
            Mod._mod = 1000003;
            var factList = new Mod[Mod._mod];
            var ifactList = new Mod[Mod._mod];
            factList[0] = 1;
            ifactList[0] = 1;
            for (int i = 1; i < Mod._mod; ++i)
            {
                factList[i] = factList[i - 1] * i;
                ifactList[i] = 1 / factList[i];
            }
            var Q = NextInt;
            for (int i = 0; i < Q; ++i)
            {
                ...
                if (d == 0)
                {
                    Console.WriteLine(Mod.Pow(x, n));
                }
                else if ((long)(x / d) + n - 1 >= Mod._mod)
                {
                    Console.WriteLine(0);
                }
                else
                {
                    var i1 = (long)(x / d + n - 1);
                    var i2 = (long)(x / d - 1);
                    var ans = Mod.Pow(d, n) * (factList[i1] * ifactList[i2]);
