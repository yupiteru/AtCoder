...
namespace Program
{
    public class M_SOLUTIONSD
    {
        static public void Solve()
        {
            var N = NextInt;
            var edge = Enumerable.Repeat(0, N).Select(_ => new List<int>()).ToList();
            for (int i = 0; i < N - 1; ++i)
            {
                var a = NextInt - 1;
                var b = NextInt - 1;
            ...
            var cList = NextIntList(N);
            RevSort(cList);

            var q = new Queue<int>();
            var ansList = new int[N];
            var dist = new int[N];
            var done = new bool[N];
            q.Enqueue(0);
            dist[0] = 0;
            ...
            Console.WriteLine(cList.Skip(1).Sum());
