...
namespace Program
{
    public static class AGC012B
    {
        ...
        static public void Solve()
        {
            ...
            var Q = NN;
            var vdc = Repeat(0, Q).Select(_ => new { v = NN - 1, d = NN, c = NN }).ToArray();
            ...
            foreach (var item in vdc.Reverse())
            {
                {
                    var q = new Queue<Tuple<long, long>>();
                    q.Enqueue(Tuple.Create(item.v, item.d));
                    while (q.Count > 0)
                    {
                        var v = q.Dequeue();
                        if (ans[v.Item1, 1] >= v.Item2) continue;
                        if (ans[v.Item1, 0] == 0) ans[v.Item1, 0] = item.c;
                        ans[v.Item1, 1] = v.Item2;
                        foreach (var item2 in path[v.Item1])
                        {
                            q.Enqueue(Tuple.Create(item2, v.Item2 - 1));
                        }
                    }
                }
            }
