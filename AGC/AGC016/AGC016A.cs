...
namespace Program
{
    public static class AGC016A
    {
        ...
        {
            var s = NS;
            var ans = 10000000000L;
            for (var i = 0; i < 26; i++)
            {
                var thisAns = 0;
                var tgt = ((char)('a' + i));
                var ary = new List<char>(s.ToArray());
                if (!ary.Contains(tgt)) continue;
                while (ary.Distinct().Count() != 1)
                {
                    ++thisAns;
                    for (var j = 1; j < s.Length; j++)
                    {
                        if (ary[j] == tgt) ary[j - 1] = tgt;
                    }
                    ary[ary.Count - thisAns] = tgt;
                }
                ans = Min(ans, thisAns);
            }
