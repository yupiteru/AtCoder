...
namespace Program
{
    public static class AGC039A
    {
        ...
        static public void Solve()
        {
            ...
            if (K == 1)
            {
                Console.WriteLine(list.Sum(e => e / 2));
            }
            else
            {
                if (list.Count == 1)
                {
                    Console.WriteLine(list.First() * K / 2);
                }
                else if (S.First() == S.Last())
                {
                    var fo = list.Take(list.Count - 1).Sum(e => e / 2);
                    var lfo = list.Skip(1).Take(list.Count - 2).Sum(e => e / 2) + (list.First() + list.Last()) / 2;
                    var l = list.Last() / 2;
                    Console.WriteLine(fo + lfo * (K - 1) + l);
                }
                else
                {
                    Console.WriteLine(list.Sum(e => e / 2) * K);
                }
            }
        }
