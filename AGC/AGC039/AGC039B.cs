...
namespace Program
{
    public static class AGC039B
    {
        ...
        static public void Solve()
        {
            ...
            var g = new Graph(N);
            var paths = new List<Tuple<int, int>>();
            for (var i = 0; i < N; i++)
            {
                for (var j = 0; j < N; j++)
                {
            ...
            {
                var allok = true;
                foreach (var item1 in path[i])
                {
                    foreach (var item2 in path[i])
                    {
                        if (SList[item1][item2] == '1')
                        {
                            allok = false;
                        }
                    }
        ...
        class Graph
        {
            public Dictionary<int, long>[] vtxPath;
            int n; List<Tuple<int, int, long>> pathList; long INF = (long.MaxValue >> 1) - 1;
