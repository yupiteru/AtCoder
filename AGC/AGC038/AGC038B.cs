...
namespace Program
{
    public static class AGC038B
    {
        ...
        static public void Solve()
        {
            ...
            var bt = new BT<long>();
            var ans = 1;
            var segTree = new SegTree<long>(N, 0, (x, y) => x + y);
            for (var i = 0; i < N - 1; i++)
            {
            ...
            for (var i = 0; i < N - K; i++)
            {
                ...
                bt.Add(PList[r]);
                var max = bt.Max();
                if (PList[l] == min && max == PList[r])
                {

                }
                else
                {
                    if (segTree.Query(l + 1, r) == 0)
                    {
                        notChangeCount = 1;
                    }
                    else
            ...
            Console.WriteLine(ans + notChangeCount);
