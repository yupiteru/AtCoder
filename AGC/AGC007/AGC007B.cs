...
namespace Program
{
    public static class AGC007B
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var pList = NNList(N);
            var aList = new long[N];
            var bList = new long[N];
            var baseNum = 500000000;
            for (var i = 0; i < N; i++)
            {
                aList[i] = baseNum + i;
                bList[i] = baseNum - i;
            }
            var aSeg = new LIB_LazySegTree<long, long>(aList, 0, 0, (x, y) => x + y, (x, y) => x + y, (x, y) => x + y);
            var bSeg = new LIB_LazySegTree<long, long>(bList, 0, 0, (x, y) => x + y, (x, y) => x + y, (x, y) => x + y);
            var num = 20001;
            foreach (var item in pList.Reverse())
            {
                var idx = item - 1;
                aSeg.Update(idx, N, num);
                if (idx + 1 < N) bSeg.Update(idx + 1, N, -num);
                num--;
            }
            for (var i = 0; i < N; i++)
            {
                if (i == 0) Console.Write(aSeg[i]);
                else Console.Write($" {aSeg[i]}");
            }
            Console.WriteLine();
            for (var i = 0; i < N; i++)
            {
                if (i == 0) Console.Write(bSeg[i]);
                else Console.Write($" {bSeg[i]}");
            }
            Console.WriteLine();
        }
