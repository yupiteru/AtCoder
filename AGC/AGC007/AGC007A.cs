...
namespace Program
{
    public static class AGC007A
    {
        ...
        static public void Solve()
        {
            ...
            var W = NN;
            var A = NSList(H);
            Func<long, long, long, long, bool> dfs = null;
            dfs = (x, y, fx, fy) =>
            {
                if (x != 0)
                {
                ...
                if (y != 0)
                {
                    if (y - 1 != fy)
                    {
                        if (A[y - 1][(int)x] == '#') return false;
                    }
                }
                if (x != W - 1 && y != H - 1)
                {
                    if (A[y][(int)x + 1] == '#' && A[y + 1][(int)x] == '#') return false;
                }
                if (x != W - 1) if (A[y][(int)x + 1] == '#') return dfs(x + 1, y, x, y);
                if (y != H - 1) if (A[y + 1][(int)x] == '#') return dfs(x, y + 1, x, y);
