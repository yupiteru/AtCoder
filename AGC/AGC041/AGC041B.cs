...
namespace Program
{
    public static class AGC041B
    {
        ...
        static public void Solve()
        {
            ...
            var P = NN;
            var AList = NNList(N);
            var sorted = AList.OrderByRand().OrderByDescending(e => e).ToArray();
            var left = 0L;
            var right = N;
            while (right - left > 1)
            {
                var mid = (right + left) / 2;
                var nokori = V - (P - 1) - (N - mid - 1) - 1;
                var kijun = sorted[mid] + M;
                var ok = true;
                if (nokori <= 0)
                {
                    if (sorted[P - 1] <= kijun) ok = true;
                    else ok = false;
                }
                else
                {
                    var total = nokori * M;
                    for (var i = mid - 1; i >= P - 1; i--)
                    {
                        total -= kijun - sorted[i];
                        if (sorted[i] > kijun)
                        {
                            ok = false;
                            break;
