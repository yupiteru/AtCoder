...
namespace Program
{
    public static class AGC031B
    {
        static public void Solve()
        {
            var N = NN;
            var CList = NNList(N);
            var colorLast = new int[200001];
            var newCList = new List<long>();
            var b = -1L;
            newCList.Add(-1);
            foreach (var item in CList)
            {
                if (b != item) newCList.Add(item);
                b = item;
            }
            var pat = new Mod[newCList.Count];
            for (var i = 1; i < newCList.Count; i++)
            {
                pat[i] = pat[i - 1];
                if (colorLast[newCList[i]] == 0)
                {
                    colorLast[newCList[i]] = i;
                    continue;
                }
                pat[i] += pat[colorLast[newCList[i]]] + 1;
                colorLast[newCList[i]] = i;
            }
            Console.WriteLine(pat.Last() + 1);
