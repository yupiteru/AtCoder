...
namespace Program
{
    public static class AGC018B
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var M = NN;
            var AList = new List<long>[N];
            for (var i = 0; i < N; i++)
            {
                AList[i] = new List<long>();
                for (var j = 0; j < M; j++)
                {
                    AList[i].Add(NN);
                }
            ...
            for (var i = 0; i < M; i++)
            {
                var dic = new Dict<long, long>(_ => 0);
                for (var j = 0; j < N; j++)
                {
                    dic[AList[j].First()]++;
                }
                var thisAns = dic.Max(e => e.Value);
                var thisKey = dic.First(e => e.Value == thisAns).Key;
                min = Min(thisAns, min);
                for (var j = 0; j < N; j++)
                {
                    AList[j].Remove(thisKey);
                }
            }
