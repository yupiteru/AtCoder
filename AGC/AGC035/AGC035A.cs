...
namespace Program
{
    public static class AGC035A
    {
        static public void Solve()
        {
            var N = NN;
            var aList = NNList(N);
            var tmp = aList.Distinct().ToArray();
            if (tmp.Length > 3)
            {
                Console.WriteLine("No");
                return;
            }
            var tmp2 = aList.GroupBy(e => e).ToDictionary(e => e.Key, e => e.Count()).Values.ToArray();

            if (tmp.Length == 3)
            {
                if (tmp2.Any(e => e != tmp2[0]))
                {
                    Console.WriteLine("No");
                    return;
                }
            ...
            if (tmp.Length == 2)
            {
                var tmp3 = tmp2.OrderBy(e => e).ToArray();
                if (tmp3[0] * 2 != tmp3[1])
                {
                    Console.WriteLine("No");
                    return;
