...
namespace Program
{
    public static class ABC001D
    {
        ...
        {
            var N = NN;
            var inputLine = NSList(N);
            var clens = inputLine.Select(e => e.Split('-').Select((x, i) =>
            {
                var tmp = "";
                var marume = true;
                if (x[3] == '0' || x[3] == '5') marume = false;
                if (int.Parse(x[3].ToString()) <= 4)
                {
                    tmp = x.Substring(0, 3) + "0";
                }
                else
                {
                    tmp = x.Substring(0, 3) + "5";
                }
                var h = int.Parse(tmp.Substring(0, 2));
                var m = int.Parse(tmp.Substring(2));
                if (i == 1 && !marume) return h * 12 + m / 5;
                if (i == 0) return h * 12 + m / 5;
                return h * 12 + m / 5 + 1;
            }).ToArray()).ToArray();
            var flag = new bool[289];
            foreach (var item in clens)
            {
            ...
            for (var i = 0; i < 289; i++)
            {
                ...
                if (flag[i] && (i == 288 || !flag[i + 1]))
                {
                    kijun.Add(i + 1);
                }
            }
            for (var i = 0; i < kijun.Count; i += 2)
            {
                var s = kijun[i];
                var e = kijun[i + 1];
                var sh = s / 12;
                ...
                var sm = (s % 12) * 5;
                var em = (e % 12) * 5;
                var ss = sh.ToString("00") + sm.ToString("00");
                var es = eh.ToString("00") + em.ToString("00");
                Console.WriteLine($"{ss}-{es}");
            }
        }
