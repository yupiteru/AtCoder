...
namespace Program
{
    public static class ABC092C
    {
        static public void Solve()
        {
            var N = NN;
            var AList = NNList(N).Concat(new[] { 0L }).ToArray();
            var allDist = new long[N + 1];
            for (var i = 0; i < AList.Length; i++)
            {
                if (i == 0) allDist[i] = Abs(AList[i]);
                else allDist[i] = Abs(AList[i - 1] - AList[i]);
            }
            var allDist2 = new long[N];
            for (var i = 0; i < AList.Length - 1; i++)
            {
                if (i == 0) allDist2[i] = Abs(AList[i + 1]);
                else allDist2[i] = Abs(AList[i - 1] - AList[i + 1]);
            }
            var allSum = allDist.Sum();
            for (var i = 0; i < N; i++)
            {
                Console.WriteLine(allSum - allDist[i] - allDist[i + 1] + allDist2[i]);
            }
        }
