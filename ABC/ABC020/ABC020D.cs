...
namespace Program
{
    public static class ABC020D
    {
        ...
        static public void Solve()
        {
            ...
            var K = NN;
            LIB_Mod ans = 0;
            foreach (var item in LIB_Math.Divisor(K).OrderBy(e => e))
            {
                var div = K / item;
                var divN = N / item;
                LIB_Mod tmp = 0;
                foreach (var item2 in LIB_Math.Divisor(div).OrderBy(e => e))
                {
                    var ary = LIB_Math.Factors(item2).ToArray();
                    if (ary.Count() != ary.Distinct().Count()) continue;
                    LIB_Mod cnt = divN / item2;
                    var size = cnt * (cnt + 1) / 2 * item * item2;
                    if (ary.Count() % 2 == 0)
                    {
                        tmp += size;
                    }
