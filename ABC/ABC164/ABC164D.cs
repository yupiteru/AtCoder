...
namespace Program
{
    public static class ABC164D
    {
        ...
        static public void Solve()
        {
            var S = NS;
            var ans = 0L;
            var revS = S.Reverse().ToArray();
            var ruiseki = new long[S.Length + 1];
            var test = 1;
            for (var i = 1; i <= S.Length; i++)
            {
                ruiseki[i] = (ruiseki[i - 1] + ((revS[i - 1] - '0') * test)) % 2019;
                test *= 10;
                test %= 2019;
            }
            var dict = new LIB_Dict<long, long>();
            dict[0]++;
            for (var i = 1; i <= S.Length; i++)
            {
                ans += dict[ruiseki[i]];
