...
namespace Program
{
    public class ABC129D
    {
        static public void Solve()
        {
            var H = NextInt;
            var W = NextInt;
            var SList = NextStringList(H);
            var lightCountW = new int[H, W];
            for (int i = 0; i < H; ++i)
            {
                var tmp = 0;
                for (int j = 0; j < W; ++j)
                ...
                for (int j = W - 2; j >= 0; --j)
                {
                    if (lightCountW[i, j] != 0 && lightCountW[i, j + 1] != 0)
                    {
                        lightCountW[i, j] = lightCountW[i, j + 1];
                    }
                }
            }
            var lightCountH = new int[H, W];
            for (int i = 0; i < W; ++i)
            {
                var tmp = 0;
                for (int j = 0; j < H; ++j)
                ...
                for (int j = H - 2; j >= 0; --j)
                {
                    if (lightCountH[j, i] != 0 && lightCountH[j + 1, i] != 0)
                    {
                        lightCountH[j, i] = lightCountH[j + 1, i];
                    }
                }
            }
            var ans = 0;
            for (int i = 0; i < H; ++i)
            {
                for (int j = 0; j < W; ++j)
                {
                    ans = Math.Max(ans, lightCountH[i, j] + lightCountW[i, j] - 1);
                }
            }
