...
namespace Program
{
    public static class ABC095D
    {
        static public void Solve()
        {
            var N = NN;
            var C = NN;
            var xv = Repeat(0, N).Select(_ => new { x = NN, v = NN }).ToArray();

            var leftMaxKarori = new List<Tuple<long/*maxkarori*/, long/*kyori*/>>();
            leftMaxKarori.Add(Tuple.Create(0L, 0L));
            var tmpSumKarori = 0L;
            foreach (var item in xv)
            {
                tmpSumKarori += item.v;
                leftMaxKarori.Add(Tuple.Create(Max(leftMaxKarori.Last().Item1, tmpSumKarori - item.x), item.x));
            }

            var rightMaxKarori = new List<Tuple<long/*maxkarori*/, long/*kyori*/>>();
            rightMaxKarori.Add(Tuple.Create(0L, 0L));
            tmpSumKarori = 0L;
            foreach (var item in xv.Reverse())
            {
                tmpSumKarori += item.v;
                rightMaxKarori.Add(Tuple.Create(Max(rightMaxKarori.Last().Item1, tmpSumKarori - (C - item.x)), C - item.x));
            }

            var ansMax = 0L;
            for (var i = 0; i <= N; i++)
            {
                ansMax = Max(ansMax, leftMaxKarori[i].Item1 + rightMaxKarori[(int)N - i].Item1 - leftMaxKarori[i].Item2);
                ansMax = Max(ansMax, rightMaxKarori[i].Item1 + leftMaxKarori[(int)N - i].Item1 - rightMaxKarori[i].Item2);
            }
