...
namespace Program
{
    public static class ABC101D
    {
        static public void Solve()
        {
            ...
            kouho = kouho.Distinct().OrderBy(e => e).ToList();
            var kouhoDic = kouho.ToDictionary(e => e, e => (double)e / e.ToString().Sum(e2 => int.Parse(e2.ToString())));
            for (var i = 0; i < kouho.Count; i++)
            {
                var ok = true;
                for (var j = i + 1; j < kouho.Count; j++)
                {
                    if (kouhoDic[kouho[i]] > kouhoDic[kouho[j]])
                    {
                        ok = false;
                        break;
                    }
