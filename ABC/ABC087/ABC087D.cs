...
namespace Program
{
    public static class ABC087D
    {
        static public void Solve()
        {
            var N = NN;
            var M = NN;
            var lrd = Repeat(0, M).Select(_ => new { L = NN - 1, R = NN - 1, D = NN }).ToArray();
            var hito = new long?[N];
            var path = Repeat(0, N).Select(_ => new List<Tuple<long, long>>()).ToArray();
            var pathNumDic = Repeat(0, N).Select(_ => new Dict<long, int>()).ToArray();
            var idx = 0;
            foreach (var item in lrd)
            {
                path[item.L].Add(Tuple.Create(item.R, item.D));
                path[item.R].Add(Tuple.Create(item.L, -item.D));
                pathNumDic[item.L][item.R] = idx;
                pathNumDic[item.R][item.L] = idx;
                ++idx;
            }
            var done = new bool[M];
            for (var i = 0; i < N; i++)
            {
                ...
                {
                    var v = q.Dequeue();
                    foreach (var temp in path[v])
                    {
                        var num = pathNumDic[v][temp.Item1];
                        if (done[num]) continue;
                        done[num] = true;
                        q.Enqueue(temp.Item1);

                        dynamic item;
                        if (temp.Item2 < 0)
                        {
                            item = new { L = temp.Item1, R = v, D = -temp.Item2 };
                        }
                        else
                        {
                            item = new { R = temp.Item1, L = v, D = temp.Item2 };
                        }
                        if (hito[item.R].HasValue)
                        {
                            if (hito[item.L].HasValue)
                            {
                                if (hito[item.L] + item.D != hito[item.R])
                                {
                            ...
                            {
                                hito[item.L] = hito[item.R] - item.D;
                            }
                        }
                        else
                        {
                            if (hito[item.L].HasValue)
                            {
                                hito[item.R] = hito[item.L] + item.D;
                            }
                            else
                            {
                                hito[item.L] = 0;
