...
namespace Program
{
    public static class ABC087C
    {
        static public void Solve()
        {
            var N = NN;
            var AList1 = NNList(N);
            var AList2 = NNList(N).Reverse().ToArray();
            var ruiseki1 = new long[N + 1];
            for (var i = 1; i <= N; i++)
            {
                ruiseki1[i] = ruiseki1[i - 1] + AList1[i - 1];
            }
            var ruiseki2 = new long[N + 1];
            for (var i = 1; i <= N; i++)
            {
                ruiseki2[i] = ruiseki2[i - 1] + AList2[i - 1];
            }
            var ans = 0L;
            for (var i = 0; i < N; i++)
            {
                ans = Max(ans, ruiseki1[i + 1] + ruiseki2[N - i]);
            }
