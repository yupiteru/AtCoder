...
namespace Program
{
    public static class ABC143E
    {
        ...
        static public void Solve()
        {
                    ...
                    while (q.Count > 0)
                    {
                        ...
                        foreach (var item in path[v])
                        {
                            ...
                            if (enery[v] < item.Item2)
                            {
                                ...
                                {
                                    eneTemp = L - item.Item2;
                                    distTemp = dist[v] + 1;
                                }
                            }
                            else
                            {
                                eneTemp = enery[v] - item.Item2;
                                distTemp = dist[v];
                            }
                            if (dist[item.Item1] > distTemp ||
                               (dist[item.Item1] == distTemp && enery[item.Item1] < eneTemp))
                            {
                                enery[item.Item1] = eneTemp;
                                dist[item.Item1] = distTemp;
                                q.Push(dist[item.Item1] * L + (L - enery[item.Item1]), item.Item1);
                            }
                        }
                    }
                    for (var j = 0; j < N; j++)
                    {
                        if (dist[j] > 1000000000000000)
                        {
                            ans[i, j] = -1;
                        }
                        else
            ...
            for (var i = 0; i < Q; i++)
            {
                Console.WriteLine(ans[queryList[i].s, queryList[i].t]);
