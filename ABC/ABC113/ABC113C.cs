    ...
    public static class ABC113C
    {
        static public void Solve()
        {
            ...
            for (int i = 0; i < M; ++i)
            {
                var P = NextString;
                var Y = NextInt;
                var Ps = new string(Repeat('0', 6 - P.Length).ToArray()) + P;
                if (!dic.ContainsKey(Ps))
                {
                    dic[Ps] = new List<int>();
                }
                dic[Ps].Add(Y);
                list.Add(Ps + "_" + Y.ToString());
            }
            var ansDic = new Dictionary<string, string>();
            foreach (var item in dic)
            {
                var tmpList = item.Value.OrderByRand().OrderBy(e => e).ToList();
                for (int i = 0; i < tmpList.Count; ++i)
                {
                    var tmp = (i + 1).ToString();
                    ansDic[item.Key + "_" + tmpList[i]] = item.Key + new string(Repeat('0', 6 - tmp.Length).ToArray()) + tmp;
                }
            }

            list.Select(e => ansDic[e]).ToList().ForEach(Console.WriteLine);
        }
