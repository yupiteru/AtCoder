    ...
    public static class ABC113D
    {
        static public void Solve()
        {
            ...
            for (int i = 1; i <= H; i++)
            {
                ...
                {
                    dp[i, j] = 0;
                    for (int k = 0; k < Pow(2, W - 1); k++)
                    {
                        var ok = true;
                        var mask = new[] { 1, 2, 4, 8, 16, 32, 64 };
                        for (int l = 0; l < W - 1; l++)
                        {
                            if (l == 0)
                            {
                                if ((k & mask[l]) != 0 && (k & mask[l + 1]) != 0)
                                {
                                    ok = false;
                                    break;
                                }
                            ...
                            {
                                if ((k & mask[l]) != 0 && ((k & mask[l - 1]) != 0 || (k & mask[l + 1]) != 0))
                                {
                                    ok = false;
                                    break;
                                }
        ...
        class Mod
        {
            ...
            private long _val = 0;
            public Mod(long x) { if ((_val = x % _mod) < 0) _val += _mod; }
