...
namespace Program
{
    public static class ABC102C
    {
        static public void Solve()
        {
            var N = NN;
            var Alist = NNList(N);
            var Alist2 = Alist.Select((x, i) => x - i - 1).OrderByRand().OrderBy(e => e).ToArray();
            var tsum = 0L;
            var numCnt = new Dictionary<long, long>();
            foreach (var item in Alist2)
            {
                if (!numCnt.ContainsKey(item))
                {
                    numCnt[item] = 0;
                }
                numCnt[item] += 1;
                tsum += Abs(Alist2.First() - item);
            }
            var ans = tsum;
            var sumCnt = 0L;
            var sumCnt2 = numCnt[Alist2.First()];
            var tmpList = Alist2.Distinct().ToArray();
            foreach (var item in tmpList.Zip(tmpList.Skip(1), (x, y) => new { x, y }))
            {
                sumCnt += numCnt[item.x];
                tsum -= (N - sumCnt2) * Abs(item.x - item.y);
                tsum += sumCnt * Abs(item.x - item.y);
                ans = Min(ans, tsum);
                sumCnt2 += numCnt[item.y];
            }
