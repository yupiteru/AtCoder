...
namespace Program
{
    public static class ABC102D
    {
        static public void Solve()
        {
            ...
            for (var i = 1; i <= N; i++)
            {
                ruiseki[i] = ruiseki[i - 1] + Alist[i - 1];
            }

            var left = new long[N + 1];
            left[2] = 1;
            var right = Repeat(N, N + 1).ToArray();
            right[N - 2] = N - 1;
            for (var i = 3; i <= N; i++)
            {
                var diffl = Abs(ruiseki[i] - ruiseki[left[i - 1]] * 2);
                var add = 0;
                while (true)
                {
                    if (diffl >= Abs(ruiseki[i] - ruiseki[left[i - 1] + add] * 2))
                    {
                        diffl = Abs(ruiseki[i] - ruiseki[left[i - 1] + add] * 2);
                        left[i] = left[i - 1] + add;
                    }
                    else
                    {
                ...
                var diffr = Abs(ruiseki[N] - ruiseki[right[N - i + 1]] * 2 + ruiseki[N - i + 1]);
                add = 0;
                while (true)
                {
                    if (diffr >= Abs(ruiseki[N] - ruiseki[right[N - i + 1] - add] * 2 + ruiseki[N - i + 1]))
                    {
                        diffr = Abs(ruiseki[N] - ruiseki[right[N - i + 1] - add] * 2 + ruiseki[N - i + 1]);
                        right[N - i] = right[N - i + 1] - add;
                    }
                    else
                    {
                        break;
            ...
            Console.WriteLine(Range(2, N - 1).Min(e =>
            {
                var P = ruiseki[left[e]];
                var Q = ruiseki[e] - ruiseki[left[e]];
                var R = ruiseki[right[e - 1]] - ruiseki[e];
                var S = ruiseki[N] - ruiseki[right[e - 1]];
                var ary = new[] { P, Q, R, S };
                return ary.Max() - ary.Min();
            }));
        }
