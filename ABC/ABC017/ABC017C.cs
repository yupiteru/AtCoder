...
namespace Program
{
    public static class ABC017C
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var M = NN;
            var lrs = Repeat(0, N).Select(_ => new { l = NN, r = NN, s = NN }).ToArray();
            var segtree = new LazySegTree<long, long>(M + 1, 0, 0, (x, y) => x + y, (x, y) => x + y, (x, y) => x + y);
            var sum = 0L;
            foreach (var item in lrs)
            {
                sum += item.s;
                segtree.Update(item.l, item.r + 1, item.s);
            }
            var ans = 0L;
            for (var i = 1; i <= M; i++)
            {
                ans = Max(ans, sum - segtree[i]);
            }
