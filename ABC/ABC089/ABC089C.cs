...
namespace Program
{
    public static class ABC089C
    {
        static public void Solve()
        {
            var N = NN;
            var SList = NSList(N);
            var sani = SList.Select(e => e[0]).Where(e => new[] { 'M', 'A', 'R', 'C', 'H' }.Contains(e)).ToArray();
            ...
            dic['C'] = 0;
            dic['H'] = 0;
            foreach (var item in sani)
            {
                dic[item]++;
            }
            var count = dic.Where(e => e.Value > 0).Count();
            var ans = 0L;
            ...
            for (var i = 0; i < 5; i++)
            {
                for (var j = i + 1; j < 5; j++)
                {
                    for (var k = j + 1; k < 5; k++)
                    {
                        ...
                        var item2 = march[j];
                        var item3 = march[k];
                        if (item1 == item2 || item2 == item3 || item1 == item3) continue;
                        ans += dic[item1] * dic[item2] * dic[item3];
                    }
                }
            }
