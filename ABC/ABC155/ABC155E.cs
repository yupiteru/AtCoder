    ...
    public static class ABC155E
    {
        ...
        static public void Solve()
        {
            ...
            for (var i = 1; i <= N.Length; i++)
            {
                var num = N[i - 1];
                dp[i, 0] = dp[i - 1, 0] + num;
                dp[i, 0] = Min(dp[i, 0], dp[i - 1, 1] + num);
                dp[i, 1] = dp[i - 1, 0] + 1 + 10 - num;
                dp[i, 1] = Min(dp[i, 1], dp[i - 1, 1] - 1 + 10 - num);
            }
            Console.WriteLine(Min(dp[N.Length, 0], dp[N.Length, 1]));
        }
