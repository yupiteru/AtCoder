...
namespace Program
{
    public static class ABC155C
    {
        ...
        {
            var N = NN;
            var SList = NSList(N).GroupBy(e => e).ToDictionary(e => e.Key, e => e.Count());
            var max = SList.Values.Max();
            foreach (var item in SList.OrderBy(e => e.Key).Where(e => e.Value == max))
            {
