...
namespace Program
{
    public static class ABC097C
    {
        static public void Solve()
        {
            ...
            var tmp2 = new SortedSet<string>();
            for (var i = 0; i < s.Length; i++)
            {
                var tmp3 = new List<string>();
                foreach (var item in tmp.Take((int)K))
                {
                    tmp3.Add(item + s[i]);
                }
                var str = new string(new[] { s[i] });
                tmp3.Add(str);
                tmp.Clear();
                tmp3.Distinct().ToList().ForEach(e =>
                {
                    tmp.Add(e);
                    tmp2.Add(e);
                });
            ...
            Console.WriteLine(tmp2.Distinct().Skip((int)K - 1).First());
        }
