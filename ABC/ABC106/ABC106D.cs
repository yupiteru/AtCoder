...
namespace Program
{
    public static class ABC106D
    {
        ...
        static public void Solve()
        {
            ...
            var M = NN;
            var Q = NN;
            var LRList = Repeat(0, M).Select(_ => new { L = NN - 1, R = NN - 1 }).ToArray();
            var pqList = Repeat(0, Q).Select(_ => new { p = NN - 1, q = NN - 1 }).ToArray();
            var seg2d = new LIB_SegTree2D<long>(N, N, 0, (x, y) => x + y);
            foreach (var item in LRList)
            {
                seg2d[item.L, item.R] += 1;
            }
            foreach (var item in pqList)
            {
                Console.WriteLine(seg2d.Query(item.p, N, 0, item.q + 1));
            }
        }
