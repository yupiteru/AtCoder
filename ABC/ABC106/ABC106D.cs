...
namespace Program
{
    public static class ABC106D
    {
        static public void Solve()
        {
            ...
            var M = NextLong;
            var Q = NextLong;
            var lrList = Repeat(0, M).Select(_ => new { L = NextLong, R = NextLong }).ToList();
            var qList = Repeat(0, Q).Select(_ => new { p = NextLong, q = NextLong }).ToList();
            var ruisekiHukanzen = new long[N + 1, N];
            var ruisekiKanzen = new long[N + 1];
            foreach (var item in lrList)
            {
                for (var i = item.R - 1; i < N; i++)
                {
                    ruisekiHukanzen[item.L, i] += 1;
                }
                ruisekiKanzen[item.R] += 1;
            }
            for (var i = 1; i <= N; i++)
            {
                for (var j = 0; j < N; j++)
                {
                    ruisekiHukanzen[i, j] += ruisekiHukanzen[i - 1, j];
                }
                ruisekiKanzen[i] += ruisekiKanzen[i - 1];
            }
            foreach (var item in qList)
            {
                Console.WriteLine(ruisekiKanzen[item.q] - ruisekiHukanzen[item.p - 1, item.q - 1]);
            }
        }
