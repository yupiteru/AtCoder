...
namespace Program
{
    public static class ABC109C
    {
        static public void Solve()
        {
            var N = NextInt;
            var X = NextLong;
            var xList = Sort(NextLongList(N).Concat(new[] { X }).ToList());
            var diffList = xList.Zip(xList.Skip(1), (x1, x2) => x2 - x1).ToList();
            if (diffList.Count == 1)
            {
                Console.WriteLine(diffList.First());
                return;
            }
            Console.WriteLine(diffList.Zip(diffList.Skip(1), (d1, d2) => GCD(d1, d2)).Min());
