...
namespace Program
{
    public static class ABC109B
    {
        static public void Solve()
        {
            var N = NextInt;
            var WList = NextStringList(N);
            if (WList.Distinct().Count() != N)
            {
                Console.WriteLine("No");

                return;
            }
            Console.WriteLine(WList.Zip(WList.Skip(1), (a, b) => new { a, b }).All(e => e.a.Last() == e.b.First()) ? "Yes" : "No");
        }
