...
namespace Program
{
    public static class ABC073B
    {
        ...
        {
            var N = NN;
            var segTree = new LazySegTree<T, long>(100000, new T { c = 1, s = 0 }, 0, (x, y) => new T { c = x.c + y.c, s = x.s + y.s }, (x, y) => new T { c = x.c, s = x.s + y * x.c }, (x, y) => x + y);
            for (var i = 0; i < N; i++)
            {
                var l = NN;
                var r = NN;
            ...
            Console.WriteLine(segTree.Query(0, 100000).s);
        }
