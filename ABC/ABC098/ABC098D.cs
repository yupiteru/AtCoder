...
namespace Program
{
    public static class ABC098D
    {
        static public void Solve()
        {
            ...
            Func<long, long, bool> addAndValid = (a, b) =>
            {
                var chkMask = 1L;
                for (var i = 0; i < 20; i++)
                {
                    var flagA = (chkMask & a) != 0;
                    var flagB = (chkMask & b) != 0;
                    if (flagA && flagB)
                    {
                        return false;
                    }
            ...
            while (rightIdx < N)
            {
                while (leftIdx < rightIdx && !addAndValid(sum, AList[rightIdx]))
                {
                    sum -= AList[leftIdx];
                    ++leftIdx;
                }
