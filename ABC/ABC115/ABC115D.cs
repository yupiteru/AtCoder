...
namespace Program
{
    public class ABC115D
    {
        static public void Solve()
        {
            ...
            var pati = new long[N + 1];
            var ban = new long[N + 1];
            pati[0] = 1;
            ban[0] = 0;
            for (int i = 1; i <= N; ++i)
            {
            ...
            while (true)
            {
                if (tgtX == ban[tgtLevel] + pati[tgtLevel])
                {
                    ans += pati[tgtLevel];
                    break;
                }
                if (tgtX > ban[tgtLevel] + pati[tgtLevel] - ban[tgtLevel - 1] - pati[tgtLevel - 1] - 1)
                {
                    ans += pati[tgtLevel - 1] + 1;
                    tgtX -= ban[tgtLevel - 1];
                    tgtX -= pati[tgtLevel - 1];
                    tgtX -= 1;
                    tgtX -= 1;
                    tgtLevel -= 1;
                    continue;
                }
                if (tgtX > ban[tgtLevel] + pati[tgtLevel] - ban[tgtLevel - 1] - pati[tgtLevel - 1] - 2)
                {
                    ans += pati[tgtLevel - 1] + 1;
                    break;
                }
                if (tgtX > ban[tgtLevel] + pati[tgtLevel] - ban[tgtLevel - 1] * 2 - pati[tgtLevel - 1] * 2 - 2)
                {
                    tgtLevel -= 1;
                    tgtX -= 1;
                    continue;
