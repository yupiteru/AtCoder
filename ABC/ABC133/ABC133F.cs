...
namespace Program
{
    public static class ABC133F
    {
        static public void Solve()
        {
            var N = NN;
            var Q = NN;
            var abcd = Repeat(0, N - 1).Select(_ => new { a = NN - 1, b = NN - 1, c = NN, d = NN }).ToArray();
            var xyuv = Repeat(0, Q).Select(_ => new { x = NN, y = NN, u = NN - 1, v = NN - 1 }).ToArray();

            var path = Repeat(0, N).Select(_ => new Dictionary<long, Tuple<long, long>>()).ToArray();
            foreach (var item in abcd)
            {
                path[item.a][item.b] = Tuple.Create(item.c, item.d);
                path[item.b][item.a] = Tuple.Create(item.c, item.d);
            }

            var tree = new Tree(path.Select(e => e.Select(e2 => e2.Key).ToList()).ToArray(), 0);
            var edgePoint = new long[N];
            var edgeSum = new long[N];
            var colorCnt = new long[N];
            var colorSum = new long[N];
            var colorRuisekiCnt = Repeat(0, N).Select(_ => new BT<Tuple<long, long>>((x, y) => x.Item1.CompareTo(y.Item1))).ToArray();
            var colorRuisekiSum = Repeat(0, N).Select(_ => new BT<Tuple<long, long>>((x, y) => x.Item1.CompareTo(y.Item1))).ToArray();
            var sum = 0L;
            var edgeNum = 0L;
            foreach (var item in tree.Euler())
            {
                if (item.Item2 == -1) continue;
                var edge = path[item.Item2][item.Item1];
                edgeNum++;
                sum += edge.Item2 * item.Item3;
                colorSum[edge.Item1] += edge.Item2 * item.Item3;
                colorCnt[edge.Item1] += item.Item3;
                if (item.Item3 == 1)
                {
                    edgeSum[item.Item1] = sum;
                    edgePoint[item.Item1] = edgeNum;
                }
                colorRuisekiCnt[edge.Item1].Add(Tuple.Create(edgeNum, colorCnt[edge.Item1]));
                colorRuisekiSum[edge.Item1].Add(Tuple.Create(edgeNum, colorSum[edge.Item1]));
            }
            ...
            foreach (var item in xyuv)
            {
                ...
                var sumDist = edgeSum[item.u] + edgeSum[item.v] - edgeSum[z] * 2;
                var colorCountU = colorRuisekiCnt[item.x].FindLower(Tuple.Create(edgePoint[item.u], 0L)).Item2?.Item2;
                var colorCountV = colorRuisekiCnt[item.x].FindLower(Tuple.Create(edgePoint[item.v], 0L)).Item2?.Item2;
                var colorCountZ = colorRuisekiCnt[item.x].FindLower(Tuple.Create(edgePoint[z], 0L)).Item2?.Item2;
                var colorCount = get(colorCountU) + get(colorCountV) - get(colorCountZ) * 2;
                var colorDistU = colorRuisekiSum[item.x].FindLower(Tuple.Create(edgePoint[item.u], 0L)).Item2?.Item2;
                var colorDistV = colorRuisekiSum[item.x].FindLower(Tuple.Create(edgePoint[item.v], 0L)).Item2?.Item2;
                var colorDistZ = colorRuisekiSum[item.x].FindLower(Tuple.Create(edgePoint[z], 0L)).Item2?.Item2;
                var colorDist = get(colorDistU) + get(colorDistV) - get(colorDistZ) * 2;

                Console.WriteLine(sumDist + colorCount * item.y - colorDist);
            }
        }
