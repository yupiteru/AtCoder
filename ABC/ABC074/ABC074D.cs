    ...
    public static class ABC074D
    {
        ...
        static public void Solve()
        {
            ...
            for (var i = 0; i < N; i++)
            {
                for (var j = 0; j < N; j++)
                {
                    graph.AddPath(i, j, A[i, j]);
                }
            }
            var warshallDist = graph.WarshallFloyd();
            for (var i = 0; i < N; i++)
            {
                for (var j = 0; j < N; j++)
                {
                    if (A[i, j] != warshallDist[i, j])
                    {
                        Console.WriteLine(-1);
                        return;
                    }
            ...
            var q = new PQ<long, Tuple<long, long>>();
            for (var i = 0L; i < N; i++)
            {
                for (var j = 0L; j < N; j++)
                {
                    if (i == j) continue;
                    q.Push(A[i, j], Tuple.Create(i, j));
                }
            }
            var uf = new UF(N);
            ...
            {
                var p = q.Pop();
                if (!uf.IsSame(p.Item2.Item1, p.Item2.Item2))
                {
                    uf.Unite(p.Item2.Item1, p.Item2.Item2);
                    ans += p.Item1;
                    for (var i = 0; i < N; i++)
                    {
                        for (var j = 0; j < N; j++)
                        {
                            var newDist = dist[i, p.Item2.Item1] + p.Item1 + dist[p.Item2.Item2, j];
                            if (dist[i, j] > newDist)
                            {
                                dist[i, j] = newDist;
                                dist[j, i] = newDist;
                ...
                {
                    if (p.Item1 < dist[p.Item2.Item1, p.Item2.Item2])
                    {
                        ans += p.Item1;
                        for (var i = 0; i < N; i++)
                        {
