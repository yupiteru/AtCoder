...
namespace Program
{
    public static class ABC159D
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var AList = NNList(N);
            var dic = AList.GroupBy(e => e).ToDictionary(e => e.Key, e => e.LongCount());
            var total = dic.Sum(e => e.Value * (e.Value - 1) / 2);
            foreach (var item in AList)
            {
                ...
                {
                    var tmp1 = dic[item] * (dic[item] - 1) / 2;
                    var tmp2 = (dic[item] - 1) * (dic[item] - 2) / 2;
                    Console.WriteLine(total - tmp1 + tmp2);
                }
            }
        }
