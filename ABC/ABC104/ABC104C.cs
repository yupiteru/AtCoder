...
namespace Program
{
    public static class ABC104C
    {
        static public void Solve()
        {
            var D = NN;
            var G = NN;
            var pc = Repeat(0, D).Select(_ => new { p = NN, c = NN }).ToArray();
            var dp = new long[D + 1, 1001];
            for (var i = 1; i <= D; i++)
            {
            ...
            for (var i = 1; i <= D; i++)
            {
                for (var j = 1000; j >= 0; j--)
                {
                    ...
                    if (dp[i - 1, j] != -1)
                    {
                        if (j <= 1000 - pc[i - 1].p)
                        {
                            dp[i, j + pc[i - 1].p] = Max(dp[i - 1, j + pc[i - 1].p], dp[i - 1, j] + pc[i - 1].c + i * 100 * pc[i - 1].p);
                        }
                        for (var k = 1; k <= pc[i - 1].p; k++)
                        {
                            if (j + k > 1000) break;
                            dp[i, j + k] = Max(Max(dp[i, j + k], dp[i - 1, j] + i * 100 * k), dp[i - 1, j + k]);
                        }
                    }
                }
            }
            Console.WriteLine(Range(1, 1000).First(e => dp[D, e] >= G));
        }
