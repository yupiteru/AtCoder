...
namespace Program
{
    public static class ABC157C
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var M = NN;
            var scList = Repeat(0, M).Select(_ => new { S = NN, C = NN }).ToArray();
            var ten = LIB_Math.Pow(10, N);
            var ten2 = LIB_Math.Pow(10, N - 1);
            if (N == 1) ten2 = 0;
            for (var i = ten2; i < ten; i++)
            {
                var ok = true;
                foreach (var item in scList)
                {
                    var num = i.ToString()[(int)item.S - 1] - '0';
                    if (num != item.C)
                    {
                        ok = false;
                        break;
