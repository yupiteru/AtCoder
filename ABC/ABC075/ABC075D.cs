...
namespace Program
{
    public static class ABC075D
    {
        ...
        static public void Solve()
        {
            ...
            var gyakuZaatuX = xyList.Select(e => e.x).OrderBy(e => e).ToArray();
            var zaatuX = gyakuZaatuX.Select((x, i) => new { x, i }).ToDictionary(e => e.x, e => e.i);
            var gyakuZaatuY = xyList.Select(e => e.y).OrderBy(e => e).ToArray();
            var zaatuY = gyakuZaatuY.Select((x, i) => new { x, i }).ToDictionary(e => e.x, e => e.i);
            var ans = long.MaxValue;
            var field = new long[gyakuZaatuX.Length + 1, gyakuZaatuY.Length + 1];
            foreach (var item in xyList)
            {
                field[zaatuX[item.x] + 1, zaatuY[item.y] + 1] += 1;
            }
            for (var i = 0; i <= gyakuZaatuX.Length; i++)
            {
                for (var j = 1; j <= gyakuZaatuY.Length; j++)
                {
                    field[i, j] += field[i, j - 1];
                }
            }
            for (var i = 0; i <= gyakuZaatuY.Length; i++)
            {
                for (var j = 1; j <= gyakuZaatuX.Length; j++)
                {
                    field[j, i] += field[j - 1, i];
                }
            }
            for (var x = 1; x <= gyakuZaatuX.Length; x++)
            {
                for (var X = x; X <= gyakuZaatuX.Length; X++)
                {
                    for (var y = 1; y <= gyakuZaatuY.Length; y++)
                    {
                        for (var Y = y; Y <= gyakuZaatuY.Length; Y++)
                        {
                            var c = field[X, Y] - field[X, y - 1] - field[x - 1, Y] + field[x - 1, y - 1];
                            if (c >= K) ans = Min(ans, (gyakuZaatuX[X - 1] - gyakuZaatuX[x - 1]) * (gyakuZaatuY[Y - 1] - gyakuZaatuY[y - 1]));
                        }
                    }
                }
            }
