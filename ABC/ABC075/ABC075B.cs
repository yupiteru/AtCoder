...
namespace Program
{
    public static class ABC075B
    {
        ...
        static public void Solve()
        {
            var H = NN;
            var W = NN;
            var bombField = new bool[H + 2, W + 2];
            for (var i = 1; i <= H; i++)
            {
                foreach (var item in NS.Select((x, ii) => new { x, ii }))
                {
                    bombField[i, item.ii + 1] = item.x == '#';
                }
            }
            var ans = new int[H + 2, W + 2];
            Func<int, int, int> calc = (h, w) =>
            {
                var ret = 0;
            ...
            for (var i = 1; i <= H; i++)
            {
                for (var j = 1; j <= W; j++)
                {
                    Console.Write((char)(bombField[i, j] ? '#' : (calc(i, j) + '0')));
                }
                Console.WriteLine();
            }
        }
