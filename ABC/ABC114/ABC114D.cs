...
namespace Program
{
    public class ABC114D
    {
        static public void Solve()
        {
            ...
            var factCounter = new Dictionary<long, long>();
            for (int i = 1; i <= N; i++)
            {
                foreach (var item in Factors(i))
                {
                    if (!factCounter.ContainsKey(item))
                    {
                        factCounter[item] = 0;
                    }
                    factCounter[item]++;
            ...
            var s3 = factCounter.Where(e => e.Value >= 2).Select(e => e.Key).ToList();
            var s5 = factCounter.Where(e => e.Value >= 4).Select(e => e.Key).ToList();
            var s15 = factCounter.Where(e => e.Value >= 14).Select(e => e.Key).ToList();
            var s25 = factCounter.Where(e => e.Value >= 24).Select(e => e.Key).ToList();
            var s75 = factCounter.Where(e => e.Value >= 74).Select(e => e.Key).ToList();
            var tmp = new List<Tuple<long, long>>();
            var ans = 0L;
            foreach (var item in s3)
            {
                foreach (var item2 in s5)
                {
                    foreach (var item3 in s5)
                    {
                        if (item == item2 || item2 == item3 || item == item3) continue;
                        tmp.Add(Tuple.Create(item, item2 * item3));
                    }
                }
            }
            ans += tmp.Distinct().Count();
            foreach (var item in s3)
            {
                foreach (var item2 in s25)
                {
            ...
            {
                foreach (var item2 in s15)
                {
                    if (item == item2) continue;
                    ++ans;
                }
            }
            foreach (var item in s75)
            {
                ++ans;
            }
