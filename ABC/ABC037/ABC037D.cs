...
namespace Program
{
    public static class ABC037D
    {
        ...
        static public void Solve()
        {
            ...
            var memo = new Mod?[H, W];
            Mod ans = 0;
            var func = Lambda<long, long, Mod>((h, w, fun) =>
            {
                if (!memo[h, w].HasValue)
                {
                    memo[h, w] = 1;
                    if (h > 0 && aMat[h, w] < aMat[h - 1, w]) memo[h, w] += fun(h - 1, w);
                    if (w > 0 && aMat[h, w] < aMat[h, w - 1]) memo[h, w] += fun(h, w - 1);
                    if (h < H - 1 && aMat[h, w] < aMat[h + 1, w]) memo[h, w] += fun(h + 1, w);
                    if (w < W - 1 && aMat[h, w] < aMat[h, w + 1]) memo[h, w] += fun(h, w + 1);
                }
                return memo[h, w].Value;
            });
            for (var i = 0; i < H; i++)
            {
                for (var j = 0; j < W; j++)
