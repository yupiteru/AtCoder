...
namespace Program
{
    public static class ABC130E
    {
        static public void Solve()
        {
            var N = NextInt;
            var M = NextInt;
            var SList = NextIntList(N);
            var TList = NextIntList(M);

            var dp = new Mod[N + 1, M + 1];
            var dps = new Mod[N + 1, M + 1];
            for (var i = 0; i <= N; i++)
            {
                for (var j = 0; j <= M; j++)
                {
            ...
            for (var i = 1; i <= N; i++)
            {
                for (var j = 1; j <= M; j++)
                {
                    ...
                    var idxJ = j - 1;
                    dp[i, j] = dp[i, j - 1];
                    if (SList[idxI] == TList[idxJ])
                    {
                        dp[i, j] = (dp[i, j] + dps[i - 1, j - 1]);
                    }
                    dps[i, j] = (dps[i - 1, j] + dp[i, j]);
                }
            }
            Mod ans = 1;
            for (var i = 1; i <= N; i++)
