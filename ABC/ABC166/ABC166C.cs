...
namespace Program
{
    public static class ABC166C
    {
        ...
        static public void Solve()
        {
            ...
            var M = NN;
            var HList = NNList(N);
            var ABList = Repeat(0, M).Select(_ => new { A = NN - 1, B = NN - 1 }).ToArray();
            var ans = new bool[N];
            foreach (var item in ABList)
            {
                if (HList[item.A] <= HList[item.B])
                {
                    ans[item.A] = true;
                }
                if (HList[item.B] <= HList[item.A])
                {
                    ans[item.B] = true;
                }
            }
            Console.WriteLine(ans.Count(e => e == false));
        }
