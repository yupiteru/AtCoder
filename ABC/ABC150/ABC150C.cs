...
namespace Program
{
    public static class ABC150C
    {
        ...
        {
            var N = NN;
            var PList = string.Join("", NNList(N));
            var QList = string.Join("", NNList(N));
            var list = LIB_Math.MakePermutation(N, false).Select(e => string.Join("", e)).OrderBy(e => e).ToList();
            var a = 0;
            var b = 0;
