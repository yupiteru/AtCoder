...
namespace Program
{
    public static class ABC150E
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var CList = NNList(N);
            var sorted = CList.OrderByRand().OrderBy(e => e).ToArray();
            LIB_Mod ans = 0L;
            var tmp2 = LIB_Mod.Pow(2, N);
            for (var i = 0; i < N; i++)
            {
                LIB_Mod tmp3 = sorted[i];
                var tmp = LIB_Mod.Pow(2, N - i - 1);
                tmp3 *= tmp + (tmp / 2) * (N - i - 1);
                tmp3 *= tmp2;
                tmp2 *= 2;
                ans += tmp3;
            }
