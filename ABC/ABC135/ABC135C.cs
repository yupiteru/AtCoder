...
namespace Program
{
    public static class ABC135C
    {
        static public void Solve()
        {
            var N = NN;
            var AList = NNList(N + 1).Reverse().ToArray();
            var BList = NNList(N).Reverse().ToArray();
            var monsCnt = 0L;
            for (var i = 0; i < N; i++)
            {
                if (i == N - 1)
                {
                    var yu2 = BList[i] + (i == 0 ? 0 : BList[i - 1]);
                    var mons2 = AList[i];
                    monsCnt += Max(0, Min(yu2, mons2));
                    BList[i] = Max(0, BList[i] - Max(0, mons2 - (i == 0 ? 0 : BList[i - 1])));
                    var yu3 = BList[i];
                    var mons3 = AList[i + 1];
                    monsCnt += Max(0, Min(yu3, mons3));
                    break;
                }
                var yu = BList[i] + (i == 0 ? 0 : BList[i - 1]);
                var mons = AList[i];
                monsCnt += Max(0, Min(yu, mons));
                BList[i] = Max(0, BList[i] - Max(0, mons - (i == 0 ? 0 : BList[i - 1])));
            }
            Console.WriteLine(monsCnt);
