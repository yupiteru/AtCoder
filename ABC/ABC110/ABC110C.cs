...
namespace Program
{
    public static class ABC110C
    {
        static public void Solve()
        {
            var S = NextString;
            var T = NextString;
            var charDicS = new Dictionary<char, HashSet<char>>();
            var charDicT = new Dictionary<char, HashSet<char>>();
            for (int i = 0; i < S.Length; i++)
            {
                if (!charDicS.ContainsKey(S[i]))
                {
                    charDicS[S[i]] = new HashSet<char>();
                }
                charDicS[S[i]].Add(T[i]);
                if (!charDicT.ContainsKey(T[i]))
                {
                    charDicT[T[i]] = new HashSet<char>();
                }
                charDicT[T[i]].Add(S[i]);
            }
            if (charDicS.Any(e => e.Value.Count > 1) || charDicT.Any(e => e.Value.Count > 1))
            {
