...
namespace Program
{
    public class ABC127F
    {
        static public void Main(string[] args)
        {
            ...
            var x2 = 0L;
            var f = 0L;
            var listA = new PQ<long>(false);
            var listB = new PQ<long>();
            for (int i = 0; i < Q; ++i)
            {
                var query = NextString;
                if (query[0] == '1')
                {
                    var a = NextLong;
                    b += NextLong;
                    ...
                    if (listA.Peek > listB.Peek)
                    {
                        ...
                        f += Math.Min(Math.Abs(x1 - a), Math.Abs(x2 - a));
                    }
                    x1 = listA.Peek;
                    x2 = listB.Peek;
                }
