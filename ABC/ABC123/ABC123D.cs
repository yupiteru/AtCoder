...
namespace Program
{
    public class ABC123D
    {
        static public void Main(string[] args)
        {
            ...
            var Z = NextInt;
            var K = NextInt;
            var Alist = NextLongList(X);
            var Blist = NextLongList(Y);
            var Clist = NextLongList(Z);

            RevSort(Alist);
            RevSort(Blist);
            RevSort(Clist);

            var queue = new PriorityQueue<long, Tuple<int, int, int>>(false);
            var mark = new HashSet<Tuple<int, int, int>>();
            queue.Push(Alist[0] + Blist[0] + Clist[0], Tuple.Create(0, 0, 0));
            mark.Add(Tuple.Create(0, 0, 0));

            for (int i = 0; i < K; ++i)
            {
                var kv = queue.Pop;
                Console.WriteLine(kv.Item1);
                var x = kv.Item2.Item1;
                var y = kv.Item2.Item2;
                var z = kv.Item2.Item3;
                var key1 = Tuple.Create(x + 1, y, z);
                var key2 = Tuple.Create(x, y + 1, z);
                var key3 = Tuple.Create(x, y, z + 1);
                if (Alist.Count > x + 1 && !mark.Contains(key1))
                {
                    mark.Add(key1);
                    queue.Push(Alist[x + 1] + Blist[y] + Clist[z], key1);
                }
                if (Blist.Count > y + 1 && !mark.Contains(key2))
                {
                    mark.Add(key2);
                    queue.Push(Alist[x] + Blist[y + 1] + Clist[z], key2);
                }
                if (Clist.Count > z + 1 && !mark.Contains(key3))
                {
                    mark.Add(key3);
                    queue.Push(Alist[x] + Blist[y] + Clist[z + 1], key3);
                }
            }
        }
