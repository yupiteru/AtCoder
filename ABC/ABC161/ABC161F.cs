...
namespace Program
{
    public static class ABC161F
    {
        ...
        static public void Solve()
        {
            ...
            foreach (var item in LIB_Math.Divisor(N))
            {
                if(item == 1) continue;
                var tmp = N;
                while (true)
                {
                    if (tmp % item == 0) tmp /= item;
                    else break;
                }
                if (tmp % item == 1) hashset.Add(item);
            }
            foreach (var item in LIB_Math.Divisor(N - 1))
            {
                if (item == 1) continue;
                hashset.Add(item);
            }
