...
namespace Program
{
    public static class ABC071C
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var AList = NNList(N);
            var ary = AList.GroupBy(e => e).Where(e => e.Count() > 1).OrderByDescending(e => e.Key).ToList();
            if (ary.Count == 0)
            {
                Console.WriteLine(0);
            }
            else if (ary[0].Count() > 3)
            {
                Console.WriteLine(ary[0].Key * ary[0].Key);
            }
            else
            {
                if (ary.Count < 2)
                ...
                {
                    Console.WriteLine(ary[0].Key * ary[1].Key);
                }
            }
        }
