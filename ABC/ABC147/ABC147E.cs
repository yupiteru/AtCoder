...
namespace Program
{
    public static class ABC147E
    {
        ...
        static public void Solve()
        {
            ...
            var dp = new LIB_Bitset[H, W];
            for (var i = 0; i < H; i++)
            {
                for (var j = 0; j < W; j++)
                {
                    var moved = (int)Abs(A[i, j] - B[i, j]);
                    dp[i, j] = new LIB_Bitset(12801);
                    if (i == 0 && j == 0)
                    {
                        dp[i, j][6400 - moved] = true;
                        dp[i, j][6400 + moved] = true;
                    }
                    if (i != 0)
                    {
