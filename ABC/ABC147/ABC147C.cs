...
namespace Program
{
    public static class ABC147C
    {
        ...
        {
            var N = NN;
            var human = Repeat(0, N).Select(_ => new Dictionary<long, long>()).ToArray();
            for (var i = 0; i < N; i++)
            {
                var A = NN;
                for (var j = 0; j < A; j++)
            ...
            for (var i = 0; i < cnt; i++)
            {
                ...
                for (var j = 0; j < N; j++)
                {
                    ...
                    {
                        foreach (var item in human[j])
                        {
                            if (LIB_BitUtil.IsSet(i, (int)item.Key) != (item.Value == 1))
                            {
                                ok = false;
                                break;
                            }
                ...
                if (ok)
                {
                    ans = Max(ans, LIB_BitUtil.CountOne(i));
                }
            }
