...
namespace Program
{
    public static class ABC107C
    {
        static public void Solve()
        {
            var N = NextInt;
            var K = NextInt;
            var xList = NextIntList(N);
            if (xList.Any(e => e == 0))
            {
                --K;
            }
            ...
            var left = Sort(xList.Where(e => e < 0).Select(e => Abs(e)).ToList());
            var right = Sort(xList.Where(e => e > 0).ToList());
            ...
            for (var i = 1; i < K; i++)
            {
                ...
                if (right.Count < K - i) continue;
                var leftmin = left[i - 1];
                var rightmin = right[K - i - 1];
                ans = Min(ans, Min(leftmin, rightmin) + leftmin + rightmin);
            }
