...
namespace Program
{
    public static class ABC107B
    {
        static public void Solve()
        {
            ...
            var allwhiterow = new List<int>();
            var allwhitecol = new List<int>();
            for (var i = 0; i < H; i++)
            {
                var notwhite = false;
                for (var j = 0; j < W; j++)
            ...
            for (var i = 0; i < H; i++)
            {
                if (allwhiterow.Contains(i)) continue;
                for (var j = 0; j < W; j++)
                {
                    if (allwhitecol.Contains(j)) continue;
                    Console.Write(masu[i, j] == 0 ? "." : "#");
