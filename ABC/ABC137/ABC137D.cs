...
namespace Program
{
    public static class ABC137D
    {
        static public void Solve()
        {
            ...
            var sortedABList = ABList.OrderByDescending(e => e.B).ToList();
            var gain = 0L;
            var bt = new BT<long>();
            for (var i = 0; i <= M; i++)
            {
            ...
            foreach (var item in sortedABList)
            {
                var canWork = bt.FindUpper(item.A);
                if (canWork.Item1)
                {
                    bt.Remove(canWork.Item2);
                    gain += item.B;
                }
            }
