...
namespace Program
{
    public static class ABC137E
    {
        static public void Solve()
        {
            ...
            var pathList = Repeat(0, N).Select(_ => new List<Tuple<long, long>>()).ToArray();
            foreach (var item in ABCList)
            {
                pathList[item.A].Add(Tuple.Create(item.B, P - item.C));
            }

            // beruman fo-do
            var dist = Repeat(1000000000000000000, N).ToArray();
            dist[0] = 0;
            for (var i = 0; i < N - 1; i++)
            {
                for (var j = 0; j < N; j++)
                {
                    foreach (var edge in pathList[j])
                    {
                        var tmpDist = dist[j] + edge.Item2;
                        if (dist[edge.Item1] > tmpDist)
                        {
                            dist[edge.Item1] = tmpDist;
                        }
                    }
                }
            }
            var ansKouho = dist[N - 1];
            for (var i = 0; i < N; i++)
            {
                for (var j = 0; j < N; j++)
                {
            ...
            if (ansKouho != dist[N - 1])
            {
                Console.WriteLine(-1);
            }
            else
            {
                Console.WriteLine(Max(0, -dist[N - 1]));
            }
        }
