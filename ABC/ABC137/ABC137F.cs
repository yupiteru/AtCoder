...
namespace Program
{
    public static class ABC137F
    {
        ...
        static public void Solve()
        {
            ...
            var aList = NNList(p);
            Mod._mod = p;
            var X = new ModPolynomial(new long[] { 1 });
            for (var i = 0; i < p; i++)
            {
                X.Mul(new ModPolynomial(new long[] { -i, 1 }));
            }
            var B = new ModPolynomial(new long[] { 0 });
            for (var i = 0; i < p; i++)
            {
                long div = 1;
                for (var j = 0; j < p; j++)
                ...
                if (div < 0) div += Mod._mod;
                var Q = (aList[i] * Mod.Inverse(div)) % Mod._mod;
                B.Add(X / new ModPolynomial(new long[] { -i, 1 }) * Q);
            }
            Console.WriteLine(string.Join(" ", B.coeff));
        }
        class ModPolynomial
        {
            public long[] coeff;
            public ModPolynomial(IEnumerable<long> c) { coeff = c.Select(e => { if (0 <= e && e < Mod._mod) return e; else { var t = e % Mod._mod; return t < 0 ? t + Mod._mod : t; } }).ToArray(); }
            static public ModPolynomial operator +(ModPolynomial x, long y) => new ModPolynomial(x.coeff.Select(e => e + y));
            static public ModPolynomial operator -(ModPolynomial x, long y) => new ModPolynomial(x.coeff.Select(e => e - y));
            static public ModPolynomial operator *(ModPolynomial x, long y) => new ModPolynomial(x.coeff.Select(e => e * y));
            static public ModPolynomial operator /(ModPolynomial x, long y) => new ModPolynomial(x.coeff.Select(e => e * Mod.Inverse(y)));
            static public ModPolynomial operator +(ModPolynomial x, ModPolynomial y)
            {
                var nc = new long[Max(x.coeff.Length, y.coeff.Length)];
                for (var i = 0; i < x.coeff.Length; i++) nc[i] = x.coeff[i];
                for (var i = 0; i < y.coeff.Length; i++) if ((nc[i] += y.coeff[i]) >= Mod._mod) nc[i] -= Mod._mod;
                return new ModPolynomial(nc);
            }
            public void Add(ModPolynomial x)
            {
                if (coeff.Length < x.coeff.Length) Array.Resize(ref coeff, x.coeff.Length);
                for (var i = 0; i < x.coeff.Length; i++) if ((coeff[i] += x.coeff[i]) >= Mod._mod) coeff[i] -= Mod._mod;
            }
            static public ModPolynomial operator -(ModPolynomial x, ModPolynomial y)
            {
                var nc = new long[Max(x.coeff.Length, y.coeff.Length)];
            ...
            public void Sub(ModPolynomial x)
            {
                if (coeff.Length < x.coeff.Length) Array.Resize(ref coeff, x.coeff.Length);
                for (var i = 0; i < x.coeff.Length; i++) if ((coeff[i] -= x.coeff[i]) >= Mod._mod) coeff[i] += Mod._mod;
            }
            static public ModPolynomial operator *(ModPolynomial x, ModPolynomial y)
            {
                var nc = new long[x.coeff.Length + y.coeff.Length - 1];
                for (var i = 0; i < x.coeff.Length; i++)
                {
                    for (var j = 0; j < y.coeff.Length; j++)
                    {
                        nc[i + j] += (x.coeff[i] * y.coeff[j]) % Mod._mod;
                        if (nc[i + j] >= Mod._mod) nc[i + j] -= Mod._mod;
                    }
                }
                return new ModPolynomial(nc);
            }
            public void Mul(ModPolynomial x)
            {
                var nc = new long[coeff.Length + x.coeff.Length - 1];
                for (var i = 0; i < coeff.Length; i++)
                {
                    for (var j = 0; j < x.coeff.Length; j++)
                    {
                        nc[i + j] += (coeff[i] * x.coeff[j]) % Mod._mod;
                        if (nc[i + j] >= Mod._mod) nc[i + j] -= Mod._mod;
                    }
                }
                coeff = nc;
            ...
            static public ModPolynomial operator /(ModPolynomial x, ModPolynomial y)
            {
                if (x.coeff.Length < y.coeff.Length) { return new ModPolynomial(new long[1]); }
                var maxDegree = x.coeff.Length - y.coeff.Length + 1;
                var nc = new long[maxDegree];
                var tnc = new long[x.coeff.Length];
                for (var i = 0; i < tnc.Length; i++) tnc[i] = x.coeff[i];
                var invLast = Mod.Inverse(y.coeff.Last());
                for (var i = 0; i < maxDegree; i++)
                {
                    nc[maxDegree - i - 1] = (tnc[tnc.Length - i - 1] * invLast) % Mod._mod;
                    for (var j = 0; j < y.coeff.Length; j++)
                    {
                        if ((tnc[tnc.Length - i - j - 1] -= (y.coeff[y.coeff.Length - j - 1] * nc[maxDegree - i - 1]) % Mod._mod) < 0) tnc[tnc.Length - i - j - 1] += Mod._mod;
                    }
                }
                return new ModPolynomial(nc);
            }
            public void Div(ModPolynomial x)
            {
                if (coeff.Length < x.coeff.Length) { coeff = new long[1]; return; }
                var maxDegree = coeff.Length - x.coeff.Length + 1;
                var nc = new long[maxDegree];
                var invLast = Mod.Inverse(x.coeff.Last());
                for (var i = 0; i < maxDegree; i++)
                {
                    nc[maxDegree - i - 1] = (coeff[coeff.Length - i - 1] * invLast) % Mod._mod;
                    for (var j = 0; j < x.coeff.Length; j++)
                    {
                        if ((coeff[coeff.Length - i - j - 1] -= (x.coeff[x.coeff.Length - j - 1] * nc[maxDegree - i - 1]) % Mod._mod) < 0) coeff[coeff.Length - i - j - 1] += Mod._mod;
                    }
                }
                coeff = nc;
            }
            static public ModPolynomial operator %(ModPolynomial x, ModPolynomial y)
            {
                ...
                var rc = new long[y.coeff.Length];
                for (var i = 0; i < rc.Length; i++) rc[i] = tnc[i];
                return new ModPolynomial(tnc);
            }
        }
