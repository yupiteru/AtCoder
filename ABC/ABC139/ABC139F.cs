...
namespace Program
{
    public static class ABC139F
    {
        ...
        static public void Solve()
        {
            ...
            var thetaList = xyList.Select(e => new { x = e.x, y = e.y, theta = Atan2(e.y, e.x) }).OrderBy(e => e.theta).ToArray();
            Func<int, int, double> calc = (s, c) =>
            {
                var nowX = 0L;
                var nowY = 0L;
                for (var i = 0; i < c; ++i)
                {
                    nowX += thetaList[(s + i) % N].x;
                    nowY += thetaList[(s + i) % N].y;
                }
                return Sqrt(nowX * nowX + nowY * nowY);
            };
            var ans = 0.0;
            for (var i = 0; i < N; i++)
            {
