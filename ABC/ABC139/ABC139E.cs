...
namespace Program
{
    public static class ABC139E
    {
        ...
        static public void Solve()
        {
            ...
            var vToIdx = new long[N, N];
            for (var i = 0; i < N; i++)
            {
                for (var j = 1; j < N; j++)
                {
                    vToIdx[i, A[i, j - 1]] = j - 1;
                }
            }
            var memo = new long[N, N];
            var calc = Lambda<long, long, long>((a, b, fun) =>
            {
                ...
                memo[a, b] = -1;
                var kijun1 = 0L;
                if (vToIdx[a, b] == 0) kijun1 = 0;
                else kijun1 = fun(A[a, vToIdx[a, b] - 1], a);
                if (kijun1 == -1) return -1;
                var kijun2 = 0L;
                if (vToIdx[b, a] == 0) kijun2 = 0;
                else kijun2 = fun(A[b, vToIdx[b, a] - 1], b);
                if (kijun2 == -1) return -1;
                memo[a, b] = Max(kijun1, kijun2) + 1;
                return memo[a, b];
            });
            var ary = Range(0, N).Select(e => calc(e, A[e, N - 2])).ToArray();
            if (ary.Contains(-1))
            {
                Console.WriteLine(-1);
            }
