...
namespace Program
{
    public static class ABC094C
    {
        static public void Solve()
        {
            var N = NN;
            var XList = NNList(N);
            var X2List = XList.OrderByRand().OrderBy(e => e).ToArray();
            var zenhan = new BT<long>();
            for (var i = 0; i < N / 2; i++)
            {
                zenhan.Add(X2List[i]);
            }
            for (var i = 0; i < N; i++)
            {
                if (zenhan.Have(XList[i]))
                {
                    Console.WriteLine(X2List[N / 2]);
                }
                else
                {
                    Console.WriteLine(X2List[N / 2 - 1]);
                }
            }
        }
