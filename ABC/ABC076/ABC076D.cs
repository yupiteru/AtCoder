...
namespace Program
{
    public static class ABC076D
    {
        ...
        static public void Solve()
        {
            ...
            var tList = NNList(N);
            var vList = NNList(N);
            var mspeed = new long[40001];
            var speed = new double[40001];
            var time = 0;
            for (var i = 0; i < N; i++)
            {
                var v = vList[i];
                for (var j = 0; j < tList[i] * 2; j++)
                {
                    mspeed[time] = v;
                    if (j == 0) mspeed[time] = Min(mspeed[time], time == 0 ? 0 : mspeed[time - 1]);
                    ++time;
                }
            }
            for (var i = 0; i < 200; i++)
            {
                var speeds = new double[40001];
                for (var j = 1; j < time; j++)
                {
                    var d1 = Abs(speed[j - 1] - speed[j]);
                    var d2 = Abs(speed[j + 1] - speed[j]);
                    if (d1 < 0.00000000001 && d2 < 0.00000000001)
                    {
                        speeds[j] = 0.5;
                    }
                }
                for (var j = 1; j < time; j++)
                {
                    speed[j] = Min(speed[j] + speeds[j], mspeed[j]);
                }
            }
            var ans = 0.0;
