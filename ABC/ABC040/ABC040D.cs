...
namespace Program
{
    public static class ABC040D
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var M = NN;
            var aby = Repeat(0, M).Select(_ => new { a = NN - 1, b = NN - 1, y = NN }).ToArray();
            var Q = NN;
            var vw = Repeat(0, Q).Select(_ => new { v = NN - 1, w = NN }).ToArray();
            // year, 0:human 1:road, index
            var events = new List<Tuple<long, int, int>>();
            for (var i = 0; i < M; i++)
            {
                events.Add(Tuple.Create(aby[i].y, 1, i));
            }
            for (var i = 0; i < Q; i++)
            {
                events.Add(Tuple.Create(vw[i].w, 0, i));
            }
            events = events.OrderByRand().OrderByDescending(e => e.Item1).ThenBy(e => e.Item2).ToList();
            var uf = new UF(N);
            var ans = new long[Q];
            foreach (var item in events)
            {
                if (item.Item2 == 0)
                {
                    ans[item.Item3] = uf.Count(vw[item.Item3].v);
                }
                else
                {
                    uf.Unite(aby[item.Item3].a, aby[item.Item3].b);
                }
            }
            for (var i = 0; i < Q; i++)
            {
