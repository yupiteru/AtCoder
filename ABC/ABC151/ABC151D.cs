...
namespace Program
{
    public static class ABC151D
    {
        ...
        static public void Solve()
        {
            ...
            var S = NSList(H);
            var maxCnt = 0L;
            Func<long, long, long, long> check = null;
            var nagasa = new long[H, W];
            check = (sx, sy, len) =>
            {
                if (sx < 0 || sx >= H || sy < 0 || sy >= W) return 0;
                if (S[sx][(int)sy] == '#') return 0;
                if (nagasa[sx, sy] <= len) return 0;
                nagasa[sx, sy] = len;
                check(sx + 1, sy, len + 1);
                check(sx, sy + 1, len + 1);
                check(sx - 1, sy, len + 1);
            ...
            for (var sx = 0; sx < H; sx++)
            {
                for (var sy = 0; sy < W; sy++)
                {
                    nagasa = new long[H, W];
                    for (var gx = 0; gx < H; gx++)
                    {
                        for (var gy = 0; gy < W; gy++)
                        {
                            nagasa[gx, gy] = 10000000000;
                        }
                    }
                    check(sx, sy, 0);
                    for (var gx = 0; gx < H; gx++)
                    {
                        for (var gy = 0; gy < W; gy++)
                        {
                            if (nagasa[gx, gy] < 1000000000) maxCnt = Max(maxCnt, nagasa[gx, gy]);
                        }
                    }
                }
            }
