...
namespace Program
{
    public static class ABC086D
    {
        static public void Solve()
        {
            var N = NN;
            var K = NN;
            var xyc = Repeat(0, N).Select(_ => new { x = NN % (2 * K), y = NN % (2 * K), c = NS }).ToArray();
            var bTable = new long[2 * K, 2 * K];
            var wTable = new long[2 * K, 2 * K];
            foreach (var item in xyc)
            {
                if (item.c == "B")
                {
                    bTable[item.x, item.y]++;
                }
                else
                {
                    wTable[item.x, item.y]++;
                }
            }
            var bRuiseki = new long[2 * K, 2 * K + 1];
            var wRuiseki = new long[2 * K, 2 * K + 1];
            for (var i = 0; i < 2 * K; i++)
            {
                for (var j = 1; j <= 2 * K; j++)
                {
                    bRuiseki[i, j] = bRuiseki[i, j - 1] + bTable[i, j - 1];
                    wRuiseki[i, j] = wRuiseki[i, j - 1] + wTable[i, j - 1];
                }
            }
            var ans = 0L;
            for (var l = 0; l < K; l++)
            {
                ...
                for (var i = 0; i < K; i++)
                {
                    bCnt += bRuiseki[i, bEndX] - bRuiseki[i, bStartX];
                    wCnt += wRuiseki[i, 2 * K] - (wRuiseki[i, bEndX] - wRuiseki[i, bStartX]);
                }
                for (var i = K; i < 2 * K; i++)
                {
                    bCnt += bRuiseki[i, 2 * K] - (bRuiseki[i, bEndX] - bRuiseki[i, bStartX]);
                    wCnt += wRuiseki[i, bEndX] - wRuiseki[i, bStartX];
                }
                for (var i = 0; i < 2 * K; i++)
                {
                    var p1 = i;
                    var p2 = (i + K) % (2 * K);
                    bCnt -= bRuiseki[p1, bEndX] - bRuiseki[p1, bStartX];
                    wCnt += wRuiseki[p1, bEndX] - wRuiseki[p1, bStartX];
                    bCnt += bRuiseki[p1, 2 * K] - (bRuiseki[p1, bEndX] - bRuiseki[p1, bStartX]);
                    wCnt -= wRuiseki[p1, 2 * K] - (wRuiseki[p1, bEndX] - wRuiseki[p1, bStartX]);
                    bCnt += bRuiseki[p2, bEndX] - bRuiseki[p2, bStartX];
                    wCnt -= wRuiseki[p2, bEndX] - wRuiseki[p2, bStartX];
                    bCnt -= bRuiseki[p2, 2 * K] - (bRuiseki[p2, bEndX] - bRuiseki[p2, bStartX]);
                    wCnt += wRuiseki[p2, 2 * K] - (wRuiseki[p2, bEndX] - wRuiseki[p2, bStartX]);
