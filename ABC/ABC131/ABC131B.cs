...
namespace Program
{
    public static class ABC131B
    {
        static public void Solve()
        {
            var N = NextInt;
            var L = NextInt;
            var ajiList = Enumerable.Range(0, N).Select(e => L + e).ToList();
            Console.WriteLine(ajiList.Sum() - ajiList[ajiList.Select((x, i) => new { x, i }).OrderBy(e => Abs(e.x)).First().i]);
        }
