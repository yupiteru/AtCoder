...
namespace Program
{
    public static class ABC036D
    {
        ...
        static public void Solve()
        {
            ...
            foreach (var item in tree.BFSLeaf())
            {
                if (item.Item2 == -1) break;
                dp[item.Item2, 0] *= dp[item.Item1, 0] + dp[item.Item1, 1];
                dp[item.Item2, 1] *= dp[item.Item1, 0];
            }
            Console.WriteLine(dp[0, 1] + dp[0, 0]);
        }
