    ...
    public static class ABC162F
    {
        ...
        static public void Solve()
        {
            ...
            for (var i = 0; i < N; i++)
            {
                for (var k = 0; k < 3; k++)
                {
                    dp[i + 1, 1, k] = dp[i, 0, k] + AList[i];
                    if (k < 2) dp[i + 1, 0, k + 1] = dp[i, 0, k];
            ...
            if (N % 2 == 1)
            {
                ret = Max(ret, dp[N - 2, 1, 0]);
                ret = Max(ret, dp[N - 1, 1, 1]);
                ret = Max(ret, dp[N - 0, 1, 2]);
            }
            else
            {
                ret = Max(ret, dp[N - 1, 1, 0]);
                ret = Max(ret, dp[N - 0, 1, 1]);
            }
