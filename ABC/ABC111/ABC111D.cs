...
namespace Program
{
    public static class ABC111D
    {
        static public void Solve()
        {
            var N = NextLong;
            var xy = Repeat(0, N).Select(_ => new { x = NextLong, y = NextLong }).ToList();
            var sampleOdd = xy.Select(e => Abs(e.x) + Abs(e.y)).First() % 2;
            if (xy.Any(e => (Abs(e.x) + Abs(e.y)) % 2 != sampleOdd))
            {
                Console.WriteLine(-1);
                return;
            }
            var udeList = new List<long>();
            var maxX = Abs(xy.Select(e => (e.x)).Max());
            var maxY = Abs(xy.Select(e => (e.y)).Max());
            var tmp = 1L;
            while (tmp <= maxX + maxY)
            {
                udeList.Add(tmp);
            ...
            udeList = udeList.OrderByDescending(e => e).ToList();
            Console.WriteLine(udeList.Count);
            Console.Write(udeList.First());
            foreach (var item in udeList.Skip(1))
            {
                Console.Write(" ");
                Console.Write(item);
            }
            ...
            foreach (var item in xy)
            {
                var pointX = 0L;
                var pointY = 0L;
                foreach (var ude in udeList)
                {
                    if (Abs(pointX - item.x) < Abs(pointY - item.y))
                    {
                        if (item.y < pointY)
                        {
                            Console.Write('D');
