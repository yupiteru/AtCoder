...
namespace Program
{
    public static class ABC111C
    {
        static public void Solve()
        {
            var n = NextLong;
            var vList = NextLongList((int)n);
            var oddCountList = new long[100001];
            var evenCountList = new long[100001];
            for (int i = 0; i < n; i++)
            {
                if (i % 2 == 0)
                {
                    oddCountList[vList[i]]++;
                }
                else
                {
                    evenCountList[vList[i]]++;
                }
            }
            if (n == 2)
            {
            ...
            var tmpodd = oddCountList.Select((e, i) => new { e, i }).OrderByDescending(e => e.e).Take(2).ToList();
            var tmpeven = evenCountList.Select((e, i) => new { e, i }).OrderByDescending(e => e.e).Take(2).ToList();
            n /= 2;
            var ans = 0L;
            if (tmpodd[0].i == tmpeven[0].i)
            {
                if (tmpodd[0].e < tmpeven[0].e)
                {
                    ans = (n - tmpodd[1].e) + (n - tmpeven[0].e);
                }
                else if (tmpodd[0].e == tmpeven[0].e)
                {
                    ans = (n - tmpodd[0].e) + (n - Max(tmpodd[1].e, tmpeven[1].e));
                }
                else
                {
                    ans = (n - tmpodd[0].e) + (n - tmpeven[1].e);
                }
            }
            else
            {
                ans = (n - tmpodd[0].e) + (n - tmpeven[0].e);
            }
