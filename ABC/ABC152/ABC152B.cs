...
namespace Program
{
    public static class ABC152B
    {
        ...
        static public void Solve()
        {
            var a = NN;
            var b = NN;
            var x = new string(Repeat(a + '0', b).Select(e => (char)e).ToArray());
            var y = new string(Repeat(b + '0', a).Select(e => (char)e).ToArray());
            if (x.CompareTo(y) > 0) Console.WriteLine(y);
