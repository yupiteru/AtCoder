...
namespace Program
{
    public static class ABC141E
    {
        ...
        static public void Solve()
        {
            ...
            long b = 1000007;
            long h = 2147483647;
            var s_hash = new long[S.Length, S.Length];
            for (var i = 0; i < S.Length; i++)
            {
                for (var j = i; j < S.Length; j++)
                {
                    if (j == 0) s_hash[i, j] = S[j];
                    else s_hash[i, j] = (s_hash[i, j - 1] * b + S[j]) % h;
                }
            }
            ...
            while (right - left > 1)
            {
                ...
                var ok = false;
                var min = new Dictionary<long, int>();
                var done = new HashSet<long>();
                for (var i = 0; i <= S.Length - mid; i++)
                {
                    var hash = s_hash[i, i + mid - 1];
