...
namespace Program
{
    public class ABC118D
    {
        static public void Main(string[] args)
        {
            var N = NextInt;
            var M = NextInt;
            var Alist = NextIntList(M);

            var mattiTable = new int[] { 2, 5, 5, 4, 5, 6, 3, 7, 6 };
            ...
            for (int i = 2; i <= N; ++i)
            {
                ...
                {
                    if (i - mattiTable[item - 1] >= 0)
                    {
                        var maxKeta = dp[i][0];
                        var beforeMaxKeta = dp[i - mattiTable[item - 1]][0];
                        if (beforeMaxKeta == 0) continue;
                        if (beforeMaxKeta == -1) beforeMaxKeta = 0;
                        var ok = false;
                        if (maxKeta == beforeMaxKeta + 1)
                        {
                            if (dp[i][maxKeta] < item)
                            {
                                ok = true;
                            }
                        }
                        else if (maxKeta < beforeMaxKeta + 1)
                        {
                            ok = true;
                        }
                        if (ok)
                        {
                            Array.Copy(dp[i - mattiTable[item - 1]], dp[i], N + 1);
                            dp[i][0] = beforeMaxKeta + 1;
                            dp[i][beforeMaxKeta + 1] = item;
                        }
                    }
                }
            }
