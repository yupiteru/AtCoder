...
namespace Program
{
    public static class ABC142D
    {
        ...
        static public void Solve()
        {
            ...
            var B = NN;
            var gcd = GCD(A, B);
            var aaa = new HashSet<long>();
            var divisorlist = Divisor(gcd).ToArray();
            var ans = 0;
            foreach (var item in divisorlist.OrderByRand().OrderBy(e => e))
            {
                if (Factors(item).Skip(1).Any())
                {
