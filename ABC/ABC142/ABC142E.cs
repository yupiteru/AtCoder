...
namespace Program
{
    public static class ABC142E
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var M = NN;
            var abList = new List<Tuple<long, long>>();
            ...
            for (var i = 0; i < M; i++)
            {
                ...
                var b = NN;
                var c = NNList(b);
                abList.Add(Tuple.Create(a, b));
                cList.Add(c.Sum(e => 1 << ((int)e - 1)));
            }
            var max = 1 << (int)N;
            var g = new Graph(max);
            for (var i = 0; i < M; i++)
            {
                for (var j = 0L; j < max; j++)
                {
                    g.AddPath(j, j | cList[i], abList[i].Item1);
                }
            }
            var res = g.Dijkstra(0);
            if (res.Item2[max - 1].HasValue)
            {
                Console.WriteLine(res.Item1[max - 1]);
            }
            else
            {
                Console.WriteLine(-1);
