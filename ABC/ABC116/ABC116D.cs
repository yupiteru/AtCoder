...
namespace Program
{
    public class ABC116D
    {
        static public void Solve()
        {
            ...
            var s = new PQ<long, long>();
            var ans = 0L;
            var netaScore = new Dictionary<long, long>();
            var netaCount = new int[N + 1];
            var netaUniques = 0;
            for (int i = 0; i < K; ++i)
            {
                var v = q.Pop();
                ans += v.Item1;
                if (++netaCount[v.Item2] == 1) ++netaUniques;
                s.Push(v.Item1, v.Item2);
            }
            netaScore[netaUniques] = ans;
            while (q.Count > 0)
            {
                var bad = true;
                while (s.Count > 0)
                ...
                if (bad) break;
                var v = q.Pop();
                while (netaCount[v.Item2] > 0 && q.Count > 0) v = q.Pop();
                if (netaCount[v.Item2] > 0 && q.Count == 0) break;
                ans += v.Item1;
                ++netaCount[v.Item2];
                netaScore[++netaUniques] = ans;
            }

            Console.WriteLine(netaScore.Select(e => e.Key * e.Key + e.Value).Max());
        }
