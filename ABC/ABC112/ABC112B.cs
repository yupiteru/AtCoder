...
namespace Program
{
    public static class ABC112B
    {
        static public void Solve()
        {
            var N = NextLong;
            var T = NextLong;
            var ct = Repeat(0, N).Select(_ => NextLongList(2)).Select(e => new { c = e[0], t = e[1] }).ToList();

            Console.WriteLine(ct.Where(e => e.t <= T).Any() ? ct.Where(e => e.t <= T).Min(e => e.c).ToString() : "TLE");
        }
