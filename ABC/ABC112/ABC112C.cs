...
namespace Program
{
    public static class ABC112C
    {
        static public void Solve()
        {
            var N = NextInt;
            var xyh = Repeat(0, N).Select(_ => new { x = NextLong, y = NextLong, h = NextLong }).ToList();
            var tmp = xyh.First(e => e.h > 0);
            for (int i = 0; i <= 100; ++i)
            {
                for (int j = 0; j <= 100; ++j)
                {
                    if (xyh.All(e => Max(e.h, 0) == Max(0, Abs(tmp.x - i) + Abs(tmp.y - j) + tmp.h - Abs(i - e.x) - Abs(j - e.y))))
                    {
                        Console.Write(i);
                        Console.Write(" ");
                        Console.Write(j);
                        Console.Write(" ");
                        Console.WriteLine(tmp.h + Abs(i - tmp.x) + Abs(j - tmp.y));
                        return;
                    }
                }
            }
