        ...
        static public void Main(string[] args)
        {
            ...
            for (int k = 0; k <= K; ++k)
            {
                ...
                for (int i = 0; i <= k; ++i)
                {
                    var leftCnt = k - i;
                    var rightCnt = i;
                    var getList = new List<long>();
                    var getMinusList = new List<long>();
                    for(int j = 0;j < leftCnt;++j) {
                        if(Vlist[j] >= 0) getList.Add(Vlist[j]);
                        else getMinusList.Add(Vlist[j]);
                    }
                    for(int j = 0;j < rightCnt;++j) {
                        if(Vlist[N - j - 1] >= 0) getList.Add(Vlist[N - j - 1]);
                        else getMinusList.Add(Vlist[N - j - 1]);
                    }
                    getMinusList.Sort();
                    for(int j = 0;j < Math.Min(retCnt, getMinusList.Count);++j) {
                        getMinusList[j] = 0;
                    }
                    ans = Math.Max(ans, getMinusList.Sum() + getList.Sum());
                }
            }
