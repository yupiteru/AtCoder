...
namespace Program
{
    public class ABC128B
    {
        ...
        {
            var N = NextInt;
            var dic = new SortedDictionary<string, List<int>>();
            var dic2 = new Dictionary<Tuple<string, int>, int>();
            for (int i = 0; i < N; ++i)
            {
                var S = NextString;
                var P = NextInt;
                dic2[Tuple.Create(S, P)] = (i + 1);
            ...
            foreach (var item in dic)
            {
                ...
                {
                    Console.WriteLine(dic2[Tuple.Create(item.Key, item2)]);
                }
            }
        }
