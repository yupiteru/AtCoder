...
namespace Program
{
    public static class ABC128E
    {
        static public void Solve()
        {
            var N = NN;
            var Q = NN;
            var stx = Repeat(0, N).Select(_ => new { s = NN, t = NN, x = NN }).ToArray();
            var dList = NNList(Q);

            var douro = new List<Tuple<long, long, long>>((int)(N * 2 + Q));
            foreach (var item in stx)
            {
                douro.Add(Tuple.Create(item.s - item.x, 1L, item.x));
                douro.Add(Tuple.Create(item.t - item.x, -1L, item.x));
            }
            foreach (var item in dList)
            {
                douro.Add(Tuple.Create(item, 2L, 0L));
            }
            var kouji = new BT<long>();
            foreach (var item in douro.OrderByRand().OrderBy(e => e.Item1).ThenBy(e => e.Item2))
            {
                if (item.Item2 == 1)
                {
                    kouji.Add(item.Item3);
                }
                if (item.Item2 == 2)
                {
                    if (kouji.Any())
                    {
                        Console.WriteLine(kouji.Min());
                    }
                    else
                    {
                        Console.WriteLine(-1);
