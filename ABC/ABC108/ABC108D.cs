...
namespace Program
{
    public static class ABC108D
    {
        static public void Solve()
        {
            var L = NextLong;
            var powers = new long[] { 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576 };
            var path = new List<Tuple<long, long, long>>();
            var max = 0L;
            var sum = 0L;
            for (var i = 20L; i >= 0; i--)
            {
                if (L < powers[i]) continue;
                max = i + 1;
                L -= powers[i];
                sum += powers[i];
                break;
            ...
            {
                path.Add(Tuple.Create(i, i + 1, 0L));
                path.Add(Tuple.Create(i, i + 1, powers[i - 1]));
            }
            for (var i = 20L; i >= 0; i--)
            {
                if (L < powers[i]) continue;
                path.Add(Tuple.Create(i + 1, max, sum));
                L -= powers[i];
                sum += powers[i];
            }
            Console.WriteLine(max + " " + path.Count);
