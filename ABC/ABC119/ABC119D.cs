...
namespace Program
{
    ...
    {
        class RevComparer : IComparer<long>
        {
        ...
        static public void Main(string[] args)
        {
            ...
            var slist = NextLongList(A);
            var tlist = NextLongList(B);
            var revslist = new List<long>(slist);
            var revtlist = new List<long>(tlist);
            slist.Sort();
            tlist.Sort();
            revslist.Sort((x, y) => y.CompareTo(x));
            revtlist.Sort((x, y) => y.CompareTo(x));
            var xlist = NextLongList(Q);
            for (int i = 0; i < Q; ++i)
            {
                var x = xlist[i];
                var sr = slist.BinarySearch(x);
                var tr = tlist.BinarySearch(x);
                var sl = revslist.BinarySearch(x, new RevComparer());
                var tl = revtlist.BinarySearch(x, new RevComparer());
                if (sr < 0) sr = ~sr;
                if (tr < 0) tr = ~tr;
                if (sl < 0) sl = ~sl;
                if (tl < 0) tl = ~tl;
                var ans = 300000000000;
                if (sr != slist.Count && tr != tlist.Count)
                {
                    ans = Math.Min(ans, Math.Max(Math.Abs(x - slist[sr]), Math.Abs(x - tlist[tr])));
                }
                if (sr != slist.Count && tl != revtlist.Count)
                {
                    ans = Math.Min(ans, Math.Abs(x - slist[sr]) * 2 + Math.Abs(x - revtlist[tl]));
                    ans = Math.Min(ans, Math.Abs(x - slist[sr]) + Math.Abs(x - revtlist[tl]) * 2);
                }
                if (sl != revslist.Count && tr != tlist.Count)
                {
                    ans = Math.Min(ans, Math.Abs(x - revslist[sl]) * 2 + Math.Abs(x - tlist[tr]));
                    ans = Math.Min(ans, Math.Abs(x - revslist[sl]) + Math.Abs(x - tlist[tr]) * 2);
                }
                if (sl != revslist.Count && tl != revtlist.Count)
                {
                    ans = Math.Min(ans, Math.Max(Math.Abs(x - revslist[sl]), Math.Abs(x - revtlist[tl])));
                }
