...
namespace Program
{
    public static class ABC025B
    {
        ...
        static public void Solve()
        {
            ...
            var A = NN;
            var B = NN;
            var sd = Repeat(0, N).Select(_ => new { s = NS, d = NN }).ToArray();
            var now = 0L;
            foreach (var item in sd)
            {
                if (item.s == "East")
                {
                    now += Min(Max(item.d, A), B);
                }
                else
                {
                    now -= Min(Max(item.d, A), B);
            ...
            if (now == 0)
            {
                Console.WriteLine($"{Abs(now)}");
            }
            else if (now < 0)
            {
                Console.WriteLine($"West {Abs(now)}");
            }
            else
            {
                Console.WriteLine($"East {Abs(now)}");
            }
        }
