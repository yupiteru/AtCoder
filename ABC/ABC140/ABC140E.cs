...
namespace Program
{
    public static class ABC140E
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var PList = NNList(N);
            var sorted = PList.Select((x, i) => new { x, i }).OrderByRand().OrderByDescending(e => e.x).ToList();
            var bt = new BT<long>();
            bt.Add(sorted.First().i);
            var ans = 0L;
            for (var i = 1; i < N; i++)
            {
                var value = sorted[i].x;
                var tgtidx = sorted[i].i;
                var right1Idx = bt.FindUpper(tgtidx, false);
                var right1 = right1Idx == bt.Count() ? -1 : bt[right1Idx];
                var right2Idx = bt.FindUpper(right1, false);
                var right2 = right2Idx == bt.Count() ? -1 : bt[right2Idx];
                var left1Idx = bt.FindLower(tgtidx, false);
                var left1 = left1Idx == bt.Count() ? -1 : bt[left1Idx];
                var left2Idx = bt.FindLower(left1, false);
                var left2 = left2Idx == bt.Count() ? -1 : bt[left2Idx];
                var rightCount = 0L;
                if (right1 != -1)
                {
                    if (right2 == -1) rightCount = N - right1;
                    else rightCount = right2 - right1;
                }
                var leftCount = 0L;
                if (left1 != -1)
                {
                    if (left2 == -1) leftCount = left1 + 1;
                    else leftCount = left1 - left2;
                }
                var multiCount = rightCount + leftCount;
                if (right1 != -1)
                {
                    multiCount += leftCount * (right1 - tgtidx - 1);
                }
                else
                {
                    multiCount += leftCount * (N - tgtidx - 1);
                }
                if (left1 != -1)
                {
                    multiCount += rightCount * (tgtidx - left1 - 1);
                }
                else
                {
                    multiCount += rightCount * (tgtidx);
                }
                ans += multiCount * value;
                bt.Add(tgtidx);
            }
