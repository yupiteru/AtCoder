...
namespace Program
{
    public static class ABC140D
    {
        ...
        static public void Solve()
        {
            ...
            var S = NS;
            var before = 'X';
            var result = new Stack<Tuple<char, long>>();
            var count = 0L;
            foreach (var item in S)
            {
                if (before != item)
                {
                    if (count != 0)
                    {
                        result.Push(Tuple.Create(before, count));
                    }
                    count = 0;
                }
                before = item;
            ...
            for (var i = 0; i < K; i++)
            {
                ...
                if (result.Count == 2)
                {
                    var a = result.Pop();
                    var b = result.Pop();
                    result.Push(Tuple.Create(a.Item1, a.Item2 + b.Item2));
                }
                else
                {
                    var a = result.Pop();
                    var b = result.Pop();
                    var c = result.Pop();
                    result.Push(Tuple.Create(a.Item1, a.Item2 + b.Item2 + c.Item2));
                }
            }
            var ans = 0L;
