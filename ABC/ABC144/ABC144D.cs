...
namespace Program
{
    public static class ABC144D
    {
        ...
        static public void Solve()
        {
            ...
            double left = 0.0;
            double right = 90.0;
            var maxTaiseki = a * a * b;
            while (Abs(right - left) > 0.00000001)
            {
                double mid = (right + left) / 2.0;
                double tate = Tan(Math.PI * mid / 180.0) * a;
                double taiseki = tate * a / 2.0 * a;
                double nokori = maxTaiseki - taiseki;
                if (tate > b - 0.00000001)
                {
                    double tate2 = Tan(Math.PI * (90.0 - mid) / 180.0) * b;
                    nokori = tate2 * b / 2.0 * a;
                }
                if (nokori < x + 0.00000001)
                {
                    right = mid;
                }
                else
