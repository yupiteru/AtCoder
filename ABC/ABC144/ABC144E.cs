...
namespace Program
{
    public static class ABC144E
    {
        ...
        static public void Solve()
        {
            ...
            var AList = NNList(N);
            var FList = NNList(N);
            var sortedA = AList.OrderByRand().OrderBy(e => e).ToList();
            var sortedF = FList.OrderByRand().OrderByDescending(e => e).ToList();
            var left = -1L;
            var right = 1000000000000000000;
            while (right - left > 1)
            {
