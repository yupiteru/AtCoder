...
namespace Program
{
    public static class ABC136D
    {
        static public void Solve()
        {
            var S = NS;
            var counts = new List<Tuple<char, long>>();
            var tmpS = ' ';
            var tmpC = 0L;
            foreach (var item in S)
            {
                if (tmpS != item)
                {
                    counts.Add(Tuple.Create(tmpS, tmpC));
                    tmpS = item;
                    tmpC = 1;
                }
                else
            ...
            for (var i = 0; i < counts.Count; i += 2)
            {
                ...
                //R
                var r = counts[i].Item2 / 2 + (counts[i].Item2 & 1);
                var l = counts[i].Item2 / 2;
                r += counts[i + 1].Item2 / 2;
                l += counts[i + 1].Item2 / 2 + (counts[i + 1].Item2 & 1);
                //L
                outdata.Add(r);
                outdata.Add(l);
                tgt += counts[i + 1].Item2;
                idx += 2;
                for (; idx < tgt; idx++)
                {
                    outdata.Add(0);
