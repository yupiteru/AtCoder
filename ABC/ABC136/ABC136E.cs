...
namespace Program
{
    public static class ABC136E
    {
        ...
        static public void Solve()
        {
            ...
            var sum = AList.Sum();
            var ans = 0L;
            foreach (var item in LIB_Math.Divisor(sum))
            {
                var sorted = AList.Select(e => e % item).Where(e => e > 0).OrderByRand().OrderBy(e => e).ToList();
                var seg1 = new LIB_SegTree<long>(sorted, 0, (x, y) => x + y);
                var seg2 = new LIB_SegTree<long>(sorted.Select(e => item - e), 0, (x, y) => x + y);
                var ok = false;
                for (var i = 0; i <= sorted.Count; i++)
                {
                    var v1 = seg1.Query(0, i);
                    var v2 = seg2.Query(i, sorted.Count);
                    if (v1 == v2 && v1 <= K)
                    {
                        ok = true;
                        break;
