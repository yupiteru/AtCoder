...
namespace Program
{
    public static class ABC145E
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var T = NN;
            var AB = Repeat(0, N).Select(_ => new { A = NN, B = NN }).OrderByRand().OrderBy(e => e.A).ToArray();

            var dp = new long[N + 1, T + 1];
            for (var i = 1; i <= N; i++)
            {
                for (var j = 1; j <= T; j++)
                {
                    if (j - AB[i - 1].A < 0) dp[i, j] = Max(dp[i - 1, j], dp[i, j - 1]);
                    else dp[i, j] = Max(dp[i - 1, j - AB[i - 1].A] + AB[i - 1].B, Max(dp[i, j - 1], dp[i - 1, j]));
                }
                for (var j = 1; j <= AB[i - 1].A; j++)
                {
                    if (T - j < 0) continue;
                    dp[i, T] = Max(Max(dp[i - 1, T], dp[i - 1, T - j] + AB[i - 1].B), dp[i, T]);
                }
            }
