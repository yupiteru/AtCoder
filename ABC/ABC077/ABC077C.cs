...
namespace Program
{
    public static class ABC077C
    {
        ...
        static public void Solve()
        {
            ...
            var BList = NNList(N).OrderByRand().OrderBy(e => e).ToArray();
            var CList = NNList(N).OrderByRand().OrderBy(e => e).ToArray();
            var aIdx = 0;
            var bIdx = 0;
            var cIdx = 0;
            var bPat = new long[N];
            ...
            while (aIdx < N)
            {
                while (bIdx < N && AList[aIdx] >= BList[bIdx])
                {
                    if (bIdx < N - 1) bPat[bIdx + 1] = bPat[bIdx];
                    ++bIdx;
                }
                if (bIdx == N) break;
                ++bPat[bIdx];
            ...
            while (bIdx < N - 1)
            {
                bPat[bIdx + 1] = bPat[bIdx];
                ++bIdx;
            }
            bIdx = 0;
            while (bIdx < N)
            {
                while (cIdx < N && BList[bIdx] >= CList[cIdx])
                {
                    if (cIdx < N - 1) cPat[cIdx + 1] = cPat[cIdx];
                    ++cIdx;
                }
                if (cIdx == N) break;
                cPat[cIdx] += bPat[bIdx];
            ...
            while (cIdx < N - 1)
            {
                cPat[cIdx + 1] = cPat[cIdx];
                ++cIdx;
            }
            Console.WriteLine(cPat.Sum());
        }
