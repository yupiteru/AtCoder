...
namespace Program
{
    public static class ABC082D
    {
        static public void Solve()
        {
            ...
            var x = NN;
            var y = NN;
            var instList = new List<Tuple<char, long>>();
            var before = s.First();
            var num = 1L;
            foreach (var item in s.Skip(1))
            {
                if (before != item)
                {
                    instList.Add(Tuple.Create(before, num));
                    num = 0;
                }
                num++;
                before = item;
            ...
            instList.Add(Tuple.Create(before, num));
            if (instList.First().Item1 == 'T')
            {
                var newInstList = new List<Tuple<char, long>>();
                newInstList.Add(Tuple.Create('F', 0L));
            ...
            posy.Add(0);
            var tate = false;
            for (var i = 1; (i + 1) < instList.Count; i += 2)
            {
                var dif = instList[i + 1].Item2;
