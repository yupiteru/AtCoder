        ...
        static public void Solve()
        {
            ...
            foreach (var item in cList)
            {
                if (item != before)
                {
                    if (cnt != 0)
                    {
                        list.Add(Tuple.Create(before, cnt));
                        cnt = 0;
                    }
                }
                ++cnt;
            ...
            if (list.First().Item1 == list.Last().Item1)
            {
                list[0] = Tuple.Create(list.First().Item1, list.First().Item2 + list.Last().Item2);
                list.RemoveAt(list.Count - 1);
            }
            Console.WriteLine(list.Max(e => (e.Item2 - 1) / 2) + 1);
