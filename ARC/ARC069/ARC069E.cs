...
namespace Program
{
    public static class ARC069E
    {
        ...
        static public void Solve()
        {
            var N = NN;
            var aList = NNList(N);
            var sorted = aList.Select((x, i) => new { x, i }).OrderByRand().OrderByDescending(e => e.x).ThenBy(e => e.i).ToList();
            var ans = new long[N];
            var q = new PQ<long>();
            for (var i = 0; i < N - 1; i++)
            {
                var dif = sorted[i].x - sorted[i + 1].x;
                q.Push(sorted[i].i);
                ans[q.Peek] += dif * (i + 1);
            }
            q.Push(sorted.Last().i);
            ans[q.Peek] += sorted.Last().x * N;
